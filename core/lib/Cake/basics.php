 ...
 */
    ...
    function getImageUrl($model, $attachment, $options)
    {
		...
		if (!empty($attachment) && !empty($attachment['amazon_s3_thumb_url'])) {
			...
			if (isPluginEnabled('HighPerformance') && Configure::read('s3.is_enabled') && !empty($attachment['amazon_s3_original_url'])) {
	...
	function getToDate($to)
	{
    	if (Configure::read('listing.days_calculation_mode') == 'Night') {
    		$to = date('Y-m-d', strtotime('+1 day', strtotime($to)));
    	}
    	return $to;
    }
    function getFromToDiff($from, $to)
	{
    	$days = (strtotime($to) - strtotime($from)) / (60*60*24);
    	if (Configure::read('listing.days_calculation_mode') == 'Day') {
    		$days++;
    	}
    	return $days;
    }
	...
	{
		if ($numbers <= 999 || Configure::read('listing.count_display_type')  == 'real') {
