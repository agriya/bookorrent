    ...
    function asset_css()
    {
        ...
        if (empty($css_include_file)) {
            ...
            if (file_exists(CSS . $filename)) {
				...
				if (Configure::read('cdn.is_cdn_enabled') && $_imagePath = Configure::read('cdn.images')) {
					$cssBuffer = str_ireplace('../img', $_imagePath . '/img', $cssBuffer);
					$cssBuffer = str_ireplace('../theme', $_imagePath . '/theme', $cssBuffer);
					$cssBuffer = str_ireplace('../font', $_imagePath . '/font', $cssBuffer);
				} elseif (Configure::read('s3.upload_static_content_enabled')) {
					$_imagePath = 'http://' . Configure::read('s3.end_point') . '/' . Configure::read('s3.bucket_name') . '/';
					if (Configure::read('s3.is_cname_enabled')) {
						$_imagePath = 'http://' . Configure::read('s3.bucket_name') . '/';
					}
					$cssBuffer = str_ireplace('../img', $_imagePath . '/img', $cssBuffer);
					$cssBuffer = str_ireplace('../font', $_imagePath . '/font', $cssBuffer);
				}
