...
class XAjaxComponent extends Component
{
	...
	function thumbnail($file_id)
	{
        ...
        if (empty($_SESSION['listing_file_info'][$file_id])) {
            header('HTTP/1.1 404 Not found');
            exit(0);
        }
        header('Content-type: '.$_SESSION['listing_file_info'][$file_id]['type']);
        header('Content-Length: ' . strlen($_SESSION['listing_file_info'][$file_id]['thumb']));
        echo $_SESSION['listing_file_info'][$file_id]['thumb'];
        exit(0);
    }
	...
	function previewImage()
    {
		...
		$_SESSION['listing_file_info'][$file_id]['type'] = get_mime($_FILES['Filedata']['tmp_name']);
		$_SESSION['listing_file_info'][$file_id]['filename'] = $_FILES['Filedata']['name'];
		$_SESSION['listing_file_info'][$file_id]['thumb'] = $imagevariable;
		...
		$_SESSION['listing_file_info'][$file_id]['original'] = $base64string;
		//extension removal
