...
// Siva: info & help
		...
		if (!empty($help)){
			if(!empty($options['help_tag']) && $options['help_tag'] === 'p')
			{
				$out['help'] = $this->Html->tag('p', $help, 'help no-mar');
				unset($options['help_tag']);
			}
			else
...
// <--
		...
		}else if ($type != 'file' && !empty($divOptions) && ($modelKey =="SellTicket") && !empty($this->fieldset['CustomPricePerNight']['validateRules'][$fieldKey])) {
			// anandam_023ac09: need to fix validation for SellTicket
			$divOptions = $this->addClass($divOptions, 'validation:' . str_replace('"', '\'', json_encode(array_reverse($this->fieldset['CustomPricePerNight']['validateRules'][$fieldKey]))));
		}	
		$out['input'] = $input;
 ...
 */
	public function radio($fieldName, $options = array(), $attributes = array()) {
		...
		$strlen = $is_more_than_one_row = 0;
		$start_row = $end_row = array();
		foreach ($options as $optValue => $optTitle) {
			$strlen += strlen($optTitle);
			if (!empty($is_last_one_is_end)) {
				$start_row[$optValue] = 1;
				$is_last_one_is_end = 0;
			}
			if ($strlen >= 60) {
				$end_row[$optValue] = $is_more_than_one_row = $is_last_one_is_end = 1;
				$strlen = 0;
			}
		}
		foreach ($options as $optValue => $optTitle) {
			...
			if ($label) {
				$label_class = null;
				if ($i == 1 || !empty($start_row[$optValue])) {
					$label_class = 'class="first-label"';
				}
				if ($i == $length || !empty($end_row[$optValue])) {
					if (!empty($label_class)) {
						$label_class = 'class="first-label last-label"';
					} else {
						$label_class = 'class="last-label"';
					}
				}
				$optTitle =  sprintf($this->Html->_tags['label'], $tagName, $label_class, $optTitle);
			}
			$allOptions = array_merge($attributes, $optionsHere);
			$out[] = $this->Html->useTag('radio', $attributes['name'], $tagName, array_diff_key($allOptions, array('name' => '', 'type' => '', 'id' => '')),$optTitle);
			$i++;
		}
		$hidden = null;
	...
	public function dateTime($fieldName, $dateFormat = 'DMY', $timeFormat = '12', $attributes = array()) {
		...
		if (!empty($attributes['value'])) {
			if (is_array($attributes['value'])) {
				...
				$min = !empty($attributes['value']['minute']) ? $attributes['value']['minute'] : '';
				$hour = !empty($attributes['value']['minute']) ? $attributes['value']['hour'] : '';
			} else {
			...
			if ($name !== null) {
				...
				if ($showParents || (!in_array($title, $parents))) {
					...
					if ($attributes['style'] === 'checkbox') {
						...
						// tmp fix for checkbox class
						$label_class = '';
						if ($this->field() == 'RoomType') {
							$label_class = 'category rt-small-' . $name;
						} elseif ($this->field() == 'PropertyType') {
							$label_class = 'category pt-small-' . $name;
						} elseif ($this->field() == 'HolidayType') {
							$label_class = 'category ht-small-' . $name;
						} elseif ($this->field() == 'Amenity') {
							$label_class = 'category am-small-' . $name;
						}
 ...
 */
    ...
    function uploader($fieldName, $options = array())
    {
        ...
        if (isset($options['uPreview'])) {
            $uPreview = $options['uPreview'];
            unset($options['uPreview']);
        }
        $uploader = '<div class="js-uploader {controller:\'' . $controller . '\',filecount:\'' . $count . '\',queuefilesize:\'' . $queuefilesize . '\',filesize:\'' . $filesize . '\',filetype:\'' . $filetype . '\',movieid:\'' . $movieid . '\',session:\'' . session_id() . '\',redirectURL:\'' . $redirectURL . '\', uPreview:\''. $uPreview.'\'}">';
