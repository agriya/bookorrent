 ...
 */
	...
	private function _permanentCache($is_flash_message) {
		$debug_mode = DEBUG;		
		if (empty($debug_mode) && PERMANENT_CACHE_CHECK === true && !in_array($_SERVER['REQUEST_METHOD'], array('POST', 'PUT', 'DELETE')) && $is_flash_message === false && $this->viewPath != 'Errors' && (!in_array($this->request->params['action'], array('add', 'edit', 'order')))) {
			...
			if (empty($cache) && !in_array($this->request->params['action'], array('login', 'register', 'admin_analytics_chart', 'admin_form_bounce_chart', 'admin_ecommerce_chart', 'admin_sources_chart'))) {
				list($cache, $cache_folder) = $this->_getCacheFilename();
			}
			if (!empty($cache)) {
				$cache .= '.gz';
