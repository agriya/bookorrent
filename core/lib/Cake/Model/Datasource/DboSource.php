	...
	public function update(Model $model, $fields = array(), $values = null, $conditions = null) {
		...
		if ($conditions != null) {
			foreach ($conditions as $field => $value) {
				if ($model->getColumnType($field) == 'boolean' && (is_int($value) || is_bool($value))) {
					$conditions[$field] = $this->boolean($value, true);
				}
			}
		}
 ...
 */
	public function fields(Model $model, $alias = null, $fields = array(), $quote = true) {
		...
		// Todo: fields are cached, so we set as language based. reg: i18N behaviour issue.
		$lang = !empty($_COOKIE['CakeCookie']['user_language']) ?  $_COOKIE['CakeCookie']['user_language'] : Configure::read('site.language');
...
// 2009-03-11 // Fix: Wasn't escaping operators such as =, <= when used with expression
							$data .= $this->__parseKey($model, trim($key), $value);
// <--
// Was:						$data .= $this->name($key) . ' = ' . $value->value;
						}
					}
...
// 2009-05-29 // commented out as it's called in debug kit
