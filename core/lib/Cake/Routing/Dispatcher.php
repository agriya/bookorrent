...
// another fix in router.php#851
		...
		$extension = (isset($request->params['ext'])) ? $request->params['ext'] : "";
        if (!empty($request->params['prefix'])) {
			$t_action = substr($request->params['action'], strlen($request->params['prefix'] . '_'));
        }
        if (($_hashSecuredActions = Configure::read('site._hashSecuredActions')) && in_array($t_action, $_hashSecuredActions) && $extension != 'json') {
