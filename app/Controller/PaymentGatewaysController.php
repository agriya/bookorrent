...
class PaymentGatewaysController extends AppController
{
    ...
    public function admin_update_status($id = null, $actionId = null)
    {
        ...
        switch ($actionId) {
            ...
            case ConstMoreAction::ItemListing:
                $this->PaymentGateway->PaymentGatewaySetting->updateAll(array(
                    'PaymentGatewaySetting.test_mode_value' => $toggle
                ) , array(
                    'PaymentGatewaySetting.payment_gateway_id' => $id,
                    'PaymentGatewaySetting.name' => 'is_enable_for_item_item_fee'
                ));
                break;

            case ConstMoreAction::ItemVerification:
                $this->PaymentGateway->PaymentGatewaySetting->updateAll(array(
                    'PaymentGatewaySetting.test_mode_value' => $toggle
                ) , array(
                    'PaymentGatewaySetting.payment_gateway_id' => $id,
                    'PaymentGatewaySetting.name' => 'is_enable_for_item_verified_fee'
                ));
                break;
            ...
            case ConstMoreAction::ItemBooking:
                $this->PaymentGateway->PaymentGatewaySetting->updateAll(array(
                    'PaymentGatewaySetting.test_mode_value' => $toggle
                ) , array(
                    'PaymentGatewaySetting.payment_gateway_id' => $id,
                    'PaymentGatewaySetting.name' => 'is_enable_for_book_a_item'
                ));
                break;
        }
        $this->set('id', $id);
