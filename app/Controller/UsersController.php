...
class UsersController extends AppController
{
    ...
    public function view($username = null)
    {
        ...
        if ($this->RequestHandler->prefers('json')) {
			$this->set('user', $user);
            $response = Cms::dispatchEvent('Controller.User.user', $this, array('user' => $user));
        }else if ($this->RequestHandler->prefers('kml')) {
            $this->set('user', $user);
        } else {
            if (empty($user)) {
                throw new NotFoundException(__l('Invalid request'));
    ...
    public function register()
    {
		...
		if ($this->RequestHandler->prefers('json') && ($this->request->is('post'))){
			if ($this->RequestHandler->prefers('json')) {
                ...
                $this->request->data['UserProfile']['State']['name'] = $this->request->data['User']['state_name'];
				$this->request->data['UserProfile']['City']['name'] = $this->request->data['User']['city_name'];
                $this->request->data['UserProfile']['Country']['id'] = $this->request->data['User']['country_id'];
                $this->request->data['UserProfile']['address'] = $this->request->data['User']['address'];
                //state and country looking
                if (!empty($this->request->data['UserProfile']['City']['name'])) {
                    $this->request->data['UserProfile']['city_id'] = !empty($this->request->data['UserProfile']['City']['id']) ? $this->request->data['UserProfile']['City']['id'] : $this->User->UserProfile->City->findOrSaveAndGetId($this->request->data['UserProfile']['City']['name'], $this->request->data['UserProfile']);
                }
                if (!empty($this->request->data['UserProfile']['Country']['id'])) {
                    $this->request->data['UserProfile']['country_id'] = $this->User->UserProfile->Country->findCountryId($this->request->data['UserProfile']['Country']['id']);
                }
                if (!empty($this->request->data['UserProfile']['State']['name'])) {
                    $this->request->data['UserProfile']['state_id'] = !empty($this->request->data['UserProfile']['State']['id']) ? $this->request->data['UserProfile']['State']['id'] : $this->User->UserProfile->State->findOrSaveAndGetId($this->request->data['UserProfile']['State']['name'], $this->request->data['UserProfile']);
                }
			}
		}
        if (Configure::read('site.launch_mode') == "Private Beta" && !empty($this->request->data) || !empty($_SESSION['invite_hash'])) {
            if (!empty($_SESSION['invite_hash'])) {
                ...
                if ($is_valid) {
					$this->set('iphone_response', array("message" =>sprintf(__l('You have submitted invitation code successfully, Welcome to %s'), Configure::read('site.name')) , "error" => 0, "flag" => 1));
		...
		if ($is_register) {
			...
			if (!empty($socialuser) && empty($this->request->data)) {
				...
				$socialuser['username'] = str_replace(' ','',$socialuser['username']);
				...
				if (empty($captcha_error)) {
					...
					if ($this->User->validates() &$this->User->UserProfile->validates()) {
						...
						if ($this->User->save($this->request->data, false)) {
							  ...
							  if(Configure::read('user.is_admin_mail_after_register')==1) {
							   ...
							   // Send e-mail to users
								...
								if (Configure::read('user.is_admin_activate_after_register')) {
									$this->set('iphone_response', array("message" => __l('You have successfully registered with our site. after administrator approval you can login to site') , "error" => 0, "flag" => 1));
									...
									$this->set('iphone_response', array("message" => __l('You have successfully registered with our site.') , "error" => 0, "flag" => 1));
                            ...
                            $is_check_membership_fee = 0;
							if (Configure::read('user.signup_fee')) {
								$is_third_party_register = 0;
                                $is_check_membership_fee = 1;
								...
								if (!empty($this->request->data['User']['is_openid_register']) || !empty($this->request->data['User']['is_linkedin_register']) || !empty($this->request->data['User']['is_google_register']) || !empty($this->request->data['User']['is_googleplus_register']) || !empty($this->request->data['User']['is_yahoo_register']) || !empty($this->request->data['User']['is_facebook_register']) || !empty($this->request->data['User']['is_twitter_register'])) {
									...
									if (Configure::read('user.signup_fee')) {
										...
										if (Configure::read('user.is_admin_activate_after_register') && Configure::read('user.is_email_verification_for_register') && empty($is_third_party_register)) {
											$this->set('iphone_response', array("message" => __l('You have successfully registered with our site you can login after email verification and administrator approval, but you can able to access all features after paying signup fee.'), "error" => 0, "flag" => 1, "user_id" => $this->User->id, "activate_hash" => $this->User->getActivateHash($this->User->id), "membership_fee" => Configure::read('user.signup_fee')));
											...
											$this->set('iphone_response', array("message" => __l('You have successfully registered with our site after administrator approval you can login to site, but you can able to access all features after paying signup fee.'), "error" => 0, "flag" => 1, "user_id" => $this->User->id, "activate_hash" => $this->User->getActivateHash($this->User->id), "membership_fee" => Configure::read('user.signup_fee')));
											...
											$this->set('iphone_response', array("message" => __l('You have successfully registered with our site you can login after email verification, but you can able to access all features after paying signup fee.'), "error" => 0, "flag" => 1, "user_id" => $this->User->id, "activate_hash" => $this->User->getActivateHash($this->User->id), "membership_fee" => Configure::read('user.signup_fee')));
											...
											$this->set('iphone_response', array("message" => __l('You have successfully registered with our site you can login now, but you can able to access all features after paying signup fee.'), "error" => 0, "flag" => 1, "user_id" => $this->User->id, "activate_hash" => $this->User->getActivateHash($this->User->id), "membership_fee" => Configure::read('user.signup_fee')));
									...
									if ($this->Auth->login($this->request->data)) {
                                        ...
                                        $ip_res = $this->regiseter_iphoneresponses($this->request->data);
                                        $this->set('iphone_response', $ip_res);
										if (!$this->RequestHandler->prefers('json')) {
									...
									if (Configure::read('user.is_email_verification_for_register')) {
										$this->set('iphone_response', array("message" => __l('You have successfully registered with our site and your activation mail has been sent to your mail inbox.'), "error" => 0, "flag" => 1));
									...
									$this->_checkOrderPlaced($this->User->id);
									if (Configure::read('user.signup_fee')) {
										$this->set('iphone_response', array("message" => __l('You have successfully registered with our site after paying membership fee only login to site.'), "error" => 0, "flag" => 1, "user_id" => $this->User->id, "activate_hash" => $this->User->getActivateHash($this->User->id),  "membership_fee" => Configure::read('user.signup_fee')));
							...
							if (Configure::read('user.is_admin_activate_after_register')) {
                                if(empty($is_check_membership_fee)){
								$this->set('iphone_response', array("message" => __l('You have successfully registered with our site. after administrator approval you can login to site'), "error" => 0, "flag" => 1));
                                }
                                $this->Session->setFlash(__l('You have successfully registered with our site. after administrator approval you can login to site') , 'default', null, 'success');
                            } else {
                            ...
                            if (!empty($this->request->data['User']['openid_user_id']) || !empty($this->request->data['User']['linkedin_user_id']) || !empty($this->request->data['User']['google_user_id']) || !empty($this->request->data['User']['googleplus_user_id']) || !empty($this->request->data['User']['facebook_user_id']) || !empty($this->request->data['User']['twitter_user_id']) || !empty($this->request->data['User']['yahoo_user_id'])) {
                                ...
                                if (Configure::read('user.is_email_verification_for_register')) {
                                    ...
                                    $this->_sendActivationMail($this->request->data['User']['email'], $user_id, $this->User->getActivateHash($user_id));
                                }
								}
                            }
						...
						if(!empty($this->request->data['User']['provider'])) {
                            ...
                            if(empty($is_check_membership_fee)){
							$this->set('iphone_response', array("message" => $flash_verfy . ' ' . __l('Verification is completed successfully. But you have to fill the following required fields to complete our registration process.'), "error" => 0, "flag" => 1));
                            }
                            ...
                            if(empty($this->User->validationErrors)){
                                $this->set('iphone_response', array("message" => __l('Your registration process is not completed. Please, try again.'), "error" => 1, "flag" => 1));
                            }else{
                                $error = '';
                                ...
                                if(isset($this->User->validationErrors['passwd'])){
                                    $error .= 'Password - Invalid Passsword';
                                }
                                $this->set('iphone_response', array("message" => $error, "error" => 1, "flag" => 1));
                            }
							$this->Session->setFlash(__l('Your registration process is not completed. Please, try again.') , 'default', null, 'error');
						}
					}
				} else {
					$this->set('iphone_response', array("message" => __l('Please enter valid captcha'), "error" => 1, "flag" => 1));
    ...
    function regiseter_iphoneresponses($data = array()){
        $user = $this->User->find('first', array(
                         'conditions' => array(
                                'User.username' => $data['User'][Configure::read('user.using_to_login')]
                                ) ,
                         'recursive' => 1
        ));
        $data['User']['password'] = crypt($data['User']['passwd'], $user['User']['password']);
        $mobile_app_hash = md5($this->_unum() . $data['User'][Configure::read('user.using_to_login') ] . $data['User']['password'] . Configure::read('Security.salt'));
        $this->User->updateAll(array(
        ...
        // Just to do for lat and lng update
        /*if (!empty($_GET['latitude']) && !empty($_GET['longtitude'])) {
            //temporary hide for update lat and long Enrtries
            $this->update_iphone_user($_GET['latitude'], $_GET['longtitude'], $obj->Auth->user('id'));
        }*/
        $iphone_small_thumb = '';
        if (!empty($user['User']['twitter_user_id']) || !empty($user['User']['facebook_user_id'])) {
            if (!empty($user['User']['twitter_user_id'])) { // Twitter modified registration: password  -> twitter user id and salt //
                $iphone_small_thumb = $user['User']['twitter_avatar_url'];
            }else if(!empty($user['User']['facebook_user_id'])){
        ...
        return array(
              ...
              'iphone_big_thumb' => $iphone_small_thumb
              );
    }
    public function iphone_social_register()
    {
        ...
        $social_type    = strtolower($this->request->data['Social']['provider']);
        $is_existing    = 0;
    ...
    public function _checkOrderPlaced($user_id)
    {
        if (!empty($_SESSION['order_id'])) {
            ...
            $itemUser = $this->User->ItemUser->find('first', array(
                'conditions' => array(
                    'ItemUser.id' => $order_id,
                ) ,
                'recursive' => -1
            ));
            if (empty($itemUser['ItemUser']['user_id']) && $this->Auth->user('id') != $itemUser['ItemUser']['owner_user_id']) {
                $data = array();
                $data['ItemUser']['id'] = $order_id;
                $data['ItemUser']['user_id'] = $user_id;
                $this->User->ItemUser->save($data, false);
    ...
    public function dashboard()
    {
        ...
        $purchase_conditions['ItemUser.user_id'] = $this->Auth->user('id');
        $purchase_conditions['NOT']['ItemUser.item_user_status_id'] = array(
            ConstItemUserStatus::Canceled,
            ConstItemUserStatus::Rejected,
            ConstItemUserStatus::Expired,
            ConstItemUserStatus::CanceledByAdmin,
        );
        $total_purchased = $this->User->ItemUser->find('first', array(
            'conditions' => $purchase_conditions,
            'fields' => array(
                'SUM(ItemUser.price) as total_amount'
            ) ,
            'recursive' => -1
        ));
        $this->set('user', $user);
        ...
        //Count Querys
        $this->set('all_post_listing_count', $user['User']['item_count']);
		$status = array(
            __l('Confirmed') => array(
                'in_progress',
                ($filter_count['User']['booking_confirmed_count']) ,
                'arrivedconfirmed'
            ) ,
            __l('Pending Host Accept') => array(
                'waiting_for_acceptance',
                $filter_count['User']['booking_waiting_for_acceptance_count'],
                'pendinghostaccept'
            ) ,
            __l('Waiting For Your Review') => array(
                'waiting_for_review',
                $filter_count['User']['booking_review_count'],
                'waitingforyourreview'
            ) ,
            __l('Completed') => array(
                'completed',
                $filter_count['User']['booking_completed_count'],
                'completed'
            ) ,
            __l('Canceled') => array(
                'canceled',
                $filter_count['User']['booking_canceled_count'],
                'cancelled'
            ) ,
            __l('Host Rejected') => array(
                'rejected',
                $filter_count['User']['booking_rejected_count'],
                'hostrejected'
            ) ,
            __l('Expired') => array(
                'expired',
                $filter_count['User']['booking_expired_count'],
                'expired'
            ) ,
            __l('Payment Pending') => array(
                'payment_pending',
                $filter_count['User']['booking_payment_pending_count'],
                'paymentpending'
            ) ,
        );
        $this->set('moreActions', $status);
        $host_all_count = ($filter_count['User']['host_expired_count']+$filter_count['User']['host_canceled_count']+$filter_count['User']['host_rejected_count']+$filter_count['User']['host_completed_count']+$filter_count['User']['host_review_count']+$filter_count['User']['host_confirmed_count']+$filter_count['User']['host_waiting_for_acceptance_count']);
        ...
        $host_status = array(
            ...
            sprintf(__l('Waiting for %s Review'), Configure::read('item.alt_name_for_booker_singular_caps')) => array(
                'waiting_for_review',
		...
		if ($this->RequestHandler->prefers('json')) {
			$response = Cms::dispatchEvent('Controller.User.Dashboard', $this, array());
		}		
    }
    public function hosting_panel()
    {
        ...
        if (!empty($user_id)) {
            ...
            $models[] = array(
                'ItemUser' => array(
                    'display' => '',
                    'conditions' => array(
                        'ItemUser.owner_user_id' => $user_id,
            ...
            foreach($models as $unique_model) {
                foreach($unique_model as $model => $fields) {
                    foreach($periods as $key => $period) {
                        ...
                        if ($model == 'ItemUser') {
                            $RevenueRecieved = $this->User->ItemUser->find('first', array(
                                'conditions' => $conditions,
                                'fields' => array(
                                    'COUNT(ItemUser.id) as total_count'
                                ) ,
                                'recursive' => -1
                            ));
		...
		if ($this->RequestHandler->prefers('json')) {
			Cms::dispatchEvent('Controller.User.HostingPanel', $this, array());
		}
    }
    ...
    {
        /* User Login Failed case */
        if ($this->RequestHandler->prefers('json') && $_REQUEST['f']) {
            $this->set('iphone_response', array("message" => $this->Auth->loginError, "error" => 1, "checkauth" => "YES"));
            Cms::dispatchEvent('Controller.User.login', $this, array());
        }
		$socialuser = $this->Session->read('socialuser');
        if (!empty($socialuser)) {
            $this->redirect(array(
        ...
        if (!empty($this->request->data)) {
            ...
            if ($this->User->validates()) {
                ...
                if ($this->Auth->login()) {
					...
					if (!empty($social_type) && $social_type != 'openid') {
						$_data['User'][$social_type . '_access_token'] = $social_profile[$social_type . '_access_token'];
					}
    ...
    public function logout()
    {
		...
		if ($this->RequestHandler->prefers('json')) {
			Cms::dispatchEvent('Controller.User.Logout', $this, array(
				'data' => array("message" =>  __l('You are now logged out of the site.'), "error" => 0)
			));
		}else{
    ...
    public function forgot_password()
    {
        ...
        if ($this->Auth->user('id')) {	
			if ($this->RequestHandler->prefers('json')) {
				Cms::dispatchEvent('Controller.User.ForgetPassword', $this, array("message" =>  __l('login user unable to request'), "error" => 1));
			}else{
				$this->redirect(Router::url('/', true));
			}
        }
        ...
        if (!empty($this->request->data)) {
            ...
            if (empty($captcha_error)) {
				if ($this->User->validates()) {
					...
					if (!empty($user['User']['email'])) {
						if (!empty($user['User']['is_openid_register']) || !empty($user['User']['is_yahoo_register']) || !empty($user['User']['is_google_register']) || !empty($user['User']['is_googleplus_register']) || !empty($user['User']['is_facebook_register']) || !empty($user['User']['is_twitter_register'])) {
							...
							if ($this->RequestHandler->prefers('json')) {
								$message = array("message" =>  __l('Failed Social User'), "error" => 1);
							}
                        } else {
                            $user = $this->User->find('first', array(
                                'conditions' => array(
							...
							if ($this->RequestHandler->prefers('json')) {
								$message = array("message" => sprintf(__l('We have sent an email to %s with further instructions.'), $this->request->data['User']['email']), "error" => 0);
							}
                        }
					} else {
						...
						if ($this->RequestHandler->prefers('json')) {
							$message = array("message" =>  __l('Please enter valid email address'), "error" => 1);
						}						
					}
					App::import('Model', 'EmailTemplate');
                    $this->EmailTemplate = new EmailTemplate();
					...
					if ($this->RequestHandler->prefers('json')) {
							Cms::dispatchEvent('Controller.User.ForgetPassword', $this, $message);
					}else{
						$this->redirect(array(
							'controller' => 'users',
							'action' => 'login'
				...
				if ($this->RequestHandler->prefers('json')) {
					Cms::dispatchEvent('Controller.User.ForgetPassword', $this, array("message" =>  __l('Please enter valid captcha'), "error" => 1));
				}else{
					$this->Session->setFlash(__l('Please enter valid captcha') , 'default', null, 'error');
				}
            }
    ...
    public function change_password($user_id = null)
    {
        ...
        if (!empty($this->request->data)) {
			// todo: json api call changes neext to fix
			if ($this->RequestHandler->prefers('json')) {
				if(!empty($this->request->data)){
					$this->request->data['User'] = $this->request->data;
				}
            ...
            if ($this->User->validates()) {
                if ($this->User->updateAll(array(
                    ...
                    if ($this->Auth->user('role_id') != ConstUserTypes::Admin && Configure::read('user.is_logout_after_change_password')) {
                        ...
                        $this->Session->setFlash(__l('Your password has been changed successfully. Please login now') , 'default', null, 'success');
						if ($this->RequestHandler->prefers('json')) {
							Cms::dispatchEvent('Controller.User.ChangePassword', $this, array(
								'data' => array("message" =>  __l('Your password has been changed successfully. Please login now'), "error" => 0)
							));
						}else{
							$this->redirect(array(
								'action' => 'login'
                    ...
                    if ($this->Auth->user('role_id') == ConstUserTypes::Admin) {
						if ($this->RequestHandler->prefers('json')) {
							Cms::dispatchEvent('Controller.User.ChangePassword', $this, array(
								'data' => array("message" =>  __l('Password has been changed successfully'), "error" => 0)
							));
						}else{
							$this->redirect(array(
								'action' => 'index'
					...
					if ($this->RequestHandler->prefers('json')) {
						Cms::dispatchEvent('Controller.User.ChangePassword', $this, array(
							'data' => array("message" =>  __l('Password could not be changed'), "error" => 1)
						));
					}
                }
            } else {
    ...
    public function facepile()
    {
		...
		if ($this->RequestHandler->prefers('json')) {
            Cms::dispatchEvent('Controller.User.Facepile', $this, array());
		}
    }	
	public function follow_friends()
    {
		...
		if (!empty($socialContacts)) {
			...
			if ($this->RequestHandler->prefers('json')) {
				$response = Cms::dispatchEvent('Controller.User.FollowFriends', $this, array());
			}
		} 
    }
