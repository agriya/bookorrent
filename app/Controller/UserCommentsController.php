...
class UserCommentsController extends AppController
{
    ...
    public function index($username = null)
    {
        ...
        if ($this->RequestHandler->prefers('json')) {
            $response = Cms::dispatchEvent('Controller.UserComments.index', $this, array());
        }		
		if(!empty($this->request->params['named']['view']) && $this->request->params['named']['view'] == 'popular') {
			$this->render('popular');
		}
    ...
    public function add()
    {
        ...
        if (!empty($this->request->data)) {
			...
			if ($this->RequestHandler->prefers('json')) {
				$this->request->data['UserComment'] = $this->request->data;
			}		
            $user = $this->UserComment->User->find('first', array(
                'conditions' => array(
            ...
            if ($this->UserComment->save($this->request->data)) {
				...
				if ($this->RequestHandler->prefers('json')) {
					$message = array("message" => __l('Recommendation has been added'), "error" => 0);
				}else{
				...
				if ($this->RequestHandler->prefers('json')) {
					$message = array("message" => __l('Recommendation could not be added. Please, try again.'), "error" => 0);
				}else{
		...
		// <-- For iPhone App code
		if ($this->RequestHandler->prefers('json')) {
			$response = Cms::dispatchEvent('Controller.UserComments.Add', $this, array(
				'message' => $message
			));
		}		
    }
    ...
    public function delete($id = null)
    {
		...
		if($status){
			...
			if ($status && $this->UserComment->delete($id)) {
				$this->set('iphone_response', array("message" => __l('Recommendation deleted'), "error" => 0));
