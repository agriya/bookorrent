...
class PagesController extends AppController
{
   ...
   public function admin_add($category_id = null)
    {
		$this->Page->Behaviors->detach('I18n');
		...
		if (!is_null($category_id)) {
			if (isPluginEnabled('Items')) {
				...
				if(!empty($category)) {
					$this->request->data['Page']['category_id'] = $category['Category']['id'];
    ...
    public function admin_edit($id = null)
    {
        ...
        if (!empty($this->request->data)) {
            ...
            $this->request->data = $this->Page->read(array('id', 'created', 'modified', 'title', 'title_es', 'content', 'content_es', 'slug', 'meta_keywords', 'meta_description', 'category_id', 'parent_id'), $id);
        }
    }
    ...
    public function view($slug = null)
    {
		...
		// <-- For iPhone App code
        if ($this->RequestHandler->prefers('json')) {
            Cms::dispatchEvent('Controller.Page.StaticPage', $this, array());
        }		
    }
