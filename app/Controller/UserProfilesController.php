...
class UserProfilesController extends AppController
{
    ...
    public function edit($user_id = null)
    {
        ...
        if ($this->RequestHandler->prefers('json') && ($this->request->is('post'))){
                ...
                $this->request->data['State']['name'] = $this->request->data['state_name'];
                $this->request->data['City']['name'] = $this->request->data['city_name'];
                $this->request->data['Habit']['Habit'] = $this->request->data['habit'];
		}
        ...
        if (!empty($this->request->data['UserProfile'])) {
            ...
            if ($this->UserProfile->User->validates() &$this->UserProfile->validates() &$this->UserProfile->User->UserAvatar->validates() && $ini_upload_error) {
                ...
                $this->request->data['UserProfile']['city_id'] = !empty($this->request->data['City']['id']) ? $this->request->data['City']['id'] : $this->UserProfile->City->findOrSaveAndGetId($this->request->data['City']['name'], $this->request->data['UserProfile']);
                $this->request->data['UserProfile']['state_id'] = !empty($this->request->data['State']['id']) ? $this->request->data['State']['id'] : $this->UserProfile->State->findOrSaveAndGetId($this->request->data['State']['name'], $this->request->data['UserProfile']);
                if ($this->UserProfile->save($this->request->data)) {
                    // @todo "Language Filter"
                    if(!empty($this->request->data['UserProfile']['language_id'])){
                    $this->UserProfile->User->Item->updateAll(array(
                        'Item.language_id' => $this->request->data['UserProfile']['language_id'],
                    ) , array(
                        'Item.user_id' => $this->request->data['UserProfile']['user_id']
                    ));}
                    if (!empty($this->request->data['UserAvatar']['filename']['name'])) {
				...
				$message = array("message" => __l('User Profile has been updated'), "error" => 1);
				...
				$message = array("message" => __l('User Profile could not be updated. Please, try again.'), "error" => 1);
			...
			if ($this->RequestHandler->prefers('json')) {
				Cms::dispatchEvent('Controller.User.UserProfileUpdate', $this, array($message));
			}else{
				$this->redirect(array(
			...
			if ($this->RequestHandler->prefers('json')) {
                ...
                $this->request->data['UserProfile']['city_name'] = $this->request->data['City']['name'];
                }
                if(!empty($this->request->data['State']['name'])){
                $this->request->data['UserProfile']['state_name'] = $this->request->data['State']['name'];
                }
                if(!empty($this->request->data['Country']['name'])){
                    $this->request->data['UserProfile']['country_name'] = $this->request->data['Country']['name'];
                }
                if(!empty($this->request->data['Language']['name'])){
                    $this->request->data['UserProfile']['language_name'] = $this->request->data['Language']['name'];
                }
				Cms::dispatchEvent('Controller.User.UserProfile', $this, array(
    ...
    {
        $this->pageTitle = __l('Profile Image');
        $this->UserProfile->User->UserAvatar->Behaviors->attach('ImageUpload', Configure::read('avatar.file'));
        if (!empty($this->request->data)) {
			...
			if ($this->UserProfile->User->validates() && $this->UserProfile->User->UserAvatar->validates() && $ini_upload_error ) {
				...
				if (!empty($this->request->data['UserAvatar']['filename']['name'])) {
					...
					$this->request->data['UserAvatar']['foreign_id'] = $this->request->data['User']['id'];
					$this->Attachment->save($this->request->data['UserAvatar']);
					$this->UserProfile->User->updateAll(array(
                        'User.attachment_id' => $this->Attachment->id,
                    ) , array(
                        'User.id' => $this->request->data['User']['id']
                    ));
				}
				$this->Session->setFlash(__l('Profile Image has been updated'), 'default', null, 'success');
                $this->redirect(array(
                    'controller' => 'user_profiles',
				...
				$this->Session->setFlash(__l('Profile Image could not be updated. Please, try again.'), 'default', null, 'error');
			}
