...
class AppController extends Controller
{
    ...
    {
		// Todo: Mobile api json post issue 
		...
		if (isPluginEnabled('SocialMarketing') && !empty($is_redirect_to_social_marketing)) {
            if ($this->Auth->user('id') && !in_array($cur_page, Configure::read('social_marketing.exception_arr')) && !empty($user) && (!$user['User']['is_skipped_fb'] || !$user['User']['is_skipped_twitter'] || !$user['User']['is_skipped_google'] || !$user['User']['is_skipped_yahoo'] )) {
        ...
        // <-- For iPhone App code
        if (!empty($_GET['key'])) {
            $response = Cms::dispatchEvent('Controller.Item.handleApp', $this);
        }
        // For iPhone App code -->
    ...
    function _redirectPOST2Named($paramNames = array())
    {
        ...
        foreach($paramNames as $paramName) {
            if (isset($this->request->data['Item'][$paramName])) { //via GET query string
				if($paramName == 'Category') {
					$category = array();
					foreach ($this->request->data['Item'][$paramName] As $categories) {
						if(!empty($categories)) {
					...
					$query_strings[$paramName] = $category;
				} else {
					$query_strings[$paramName] = $this->request->data['Item'][$paramName];
				}
            } elseif (isset($this->request->data['Request'][$paramName])) {
                if($paramName == 'Category') {
					$category = array();
					foreach ($this->request->data['Request'][$paramName] As $categories) {
						if(!empty($categories)) {
							foreach ($categories As $key => $val) {
								$category[] = $val;
							}
        ...
        if (!empty($query_strings)) {
            ...
            foreach($query_strings as $key => $val) {
                if (($key == 'from' || $key == 'to') && is_array($val)) {
    ...
    function _redirectPOST2NamedJson($paramNames = array())
    {
        //redirect the URL with query string to namedArg like URL structure...
        $query_strings = array();
        foreach($paramNames as $paramName) {
        ...
        if (!empty($query_strings)) {
            ...
            //salt mixing
            $salt = $keyword_id + 786;
            return $hash_query_string = dechex($keyword_id) . '/' . substr(dechex($salt), 0, 2);
        }
    }
    ...
    function admin_update_status($id = null, $action = null)
    {
        ...
        if (!empty($this->request->params['named']['featured']) && ($this->request->params['named']['featured'] == 'deactivate')) {
            $field = 'is_featured';
            $value = 0;
            $success_message = __l('Selected record has been marked as not featured successfully');
        } elseif (!empty($this->request->params['named']['featured']) && ($this->request->params['named']['featured'] == 'activate')) {
            $field = 'is_featured';
            $value = 1;
            $success_message = __l('Selected record has been marked as featured successfully');
            ...
            $field = 'admin_suspend';
            $value = 1;
			if ($this->modelClass == 'Item' && $this->Auth->user('role_id') == ConstUserTypes::Admin) {
				$get_orders = $this->Item->find('first', array(
					'conditions' => array(
						'Item.id' => $id,
					) ,
					'contain' => array(
				...
				if (!empty($get_orders['ItemUser'])) {
					foreach($get_orders['ItemUser'] as $item_users) {
						$this->ItemUser->updateStatus($item_users['id'], ConstItemUserStatus::CanceledByAdmin);
					}
				}
			}
            ...
            $field = 'is_approved';
            $value = 1;
            if ($this->modelClass == 'Item') {
				$this->Item->autofacebookpost($id);
                ...
                if (!empty($mail_template)) {
                    ...
                    if (Configure::read('messages.is_send_internal_message')) {
                        $message_id = $this->Message->sendNotifications($item['User']['id'], $subject, $message, 0, $is_review = 0, $item['Item']['id'], 0);
        ...
        } elseif ($this->modelClass == 'Item' && (isset($this->request->params['named']['verify']) && $this->request->params['named']['verify'] == 'active')) {
                ...
                switch ($actionid) {
                    ...
                    case ConstMoreAction::Suspend:
                        ...
                        if ($this->modelClass == 'Item') {
                            foreach($ids as $id) {
                                ...
                                if (!empty($get_orders['ItemUser'])) {
                                    foreach($get_orders['ItemUser'] as $item_user) {
										$this->ItemUser->updateStatus($item_users['id'], ConstItemUserStatus::CanceledByAdmin);
                                    }
                                }
                            }
                    ...
                    case ConstMoreAction::Delete:
                        ...
                        if (!empty($is_not_deleted) && !empty($is_deleted)) {
                            $this->Session->setFlash(sprintf(__l('Checked records has been deleted. Some records are not deleted due to %s was assigned to the cancellation policy.'), Configure::read('item.alt_name_for_item_plural_small')) , 'default', null, 'success');
                        } elseif (!empty($is_not_deleted)) {
                            $this->Session->setFlash(sprintf(__l('No records deleted. Some %s was assigned to the cancellation policy.'), Configure::read('item.alt_name_for_item_plural_small')) , 'default', null, 'error');
                        } else {
    ...
    public function updateactions($id = null, $action = null)
    {
        ...
        $valid = $this->
        {
            ...
            if (empty($valid)) {
				if ($this->RequestHandler->prefers('json')) {
					...
					$this->set('iphone_response', $message);
				}else{
            ...
            if (!empty($action) && $action == 'inactive') {
				...
				if($this->request->params['controller'] != 'items'){
	                $this->Session->setFlash($this->modelClass . ' ' . __l(' has been disabled') , 'default', null, 'success');					
				}
				$message = array("message" => $this->modelClass . ' ' . __l(' has been disabled'), "error" => 0);
            } elseif (!empty($action) && $action == 'active') {
				if ($this->request->params['controller'] == 'items' && isPluginEnabled('Sudopay')) {
	                ...
	                $is_active = empty($connected_gateways) ? 0 : 1 ;
				} else {
					$is_active = 1;
				}
                $this->{$this->modelClass}->updateAll(array(
                    $this->modelClass . '.is_active' => $is_active
                ) , array(
                    $this->modelClass . '.id' => $id
                ));
				if($this->request->params['controller'] != 'items'){
			        $this->Session->setFlash($this->modelClass . ' ' . __l(' has been enabled') , 'default', null, 'success');
					$message = array("message" => $this->modelClass . ' ' . __l(' has been enabled'), "error" => 0);
				} 
				if($this->request->params['controller'] == 'items' && isPluginEnabled('Sudopay') && empty($is_active)) {
					$this->Session->setFlash(sprintf(__l('%s not enabled, You have to connect atleast one payment gateway'), Configure::read('item.alt_name_for_item_singular_caps')) , 'default', null, 'error');
					$message = array("message" => sprintf(__l('%s not enabled, You have to connect atleast one payment gateway'), Configure::read('item.alt_name_for_item_singular_caps')), "error" => 1);
					if (!$this->RequestHandler->prefers('json')) {
						$this->redirect(array(
							'controller' => 'sudopays',
                ...
                $this->Session->setFlash($this->modelClass . ' ' . __l(' has been sent for verified') , 'default', null, 'success');
				$message = array("message" => $this->modelClass . ' ' . __l(' has been sent for verified'), "error" => 0);
            } elseif (!empty($action) && $action == 'delete') {
                $this->{$this->modelClass}->deleteAll(array(
                    $this->modelClass . '.id' => $id
                ));
                $this->Session->setFlash($this->modelClass . ' ' . __l(' has been deleted') , 'default', null, 'success');
				$message = array("message" => $this->modelClass . ' ' . __l(' has been deleted'), "error" => 0);
            }
	        // <-- For iPhone App code
			if ($this->RequestHandler->prefers('json')) {
				$response = Cms::dispatchEvent('Controller.Update.UpdateActions', $this, array(
					'message' => $message
				));
			}else{			
        ...
        function update_itemUserStatus($id = null, $status_id = null)
        {
            $r = $_GET['r'];
            if (!empty($status_id)) {
                $this->{$this->modelClass}->updateAll(array(
                    $this->modelClass . '.item_user_status_id' => $status_id
                ) , array(
                    $this->modelClass . '.id' => $id
                ));
