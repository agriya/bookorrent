    ...
    function recount(Model $Model, $itemId) 
    {
        ...
        if ($Model->name == 'Job') {
            $itemStats = $Model->find('first', array(
                'fields' => array(
                    "SUM({$db->startQuote}{$Model->alias}{$db->endQuote}.{$db->startQuote}$field_rating{$db->endQuote}) totalRatings",
			...
			$postive_count = $Model->find('count', array(
				'conditions' => array(
                    "{$Model->alias}.$field_itemId" => $itemId,
                    "{$Model->alias}.$field_rating" => 1
                ) ,
                'recursive' => -1,
			));
			$meanRating = 0;
			if($postive_count > 0 && $itemStats[0]['ratingscount'] > 0) {
				$meanRating = ($postive_count / $itemStats[0]['ratingscount']);
			}
			$itemStats[0]['meanRating'] = $meanRating;
        }
        ...
        if (!$C || !$m) {
            ...
            if (!$cache['data'] || time() -$cache['data']['time'] > $cache['calculationDuration']) {
                ...
                $allStats = $ItemModel->find('first', array(
                    'fields' => array(
                        "AVG({$db->startQuote}{$ItemModel->alias}{$db->endQuote}.{$db->startQuote}{$fields['ratingsCount']}{$db->endQuote}) C",
                        "AVG({$db->startQuote}{$ItemModel->alias}{$db->endQuote}.{$db->startQuote}{$fields['meanRating']}{$db->endQuote}) m"
                    ) ,
                    'conditions' => array(
                        "{$ItemModel->alias}.{$fields['ratingsCount']} >" => '0'
                    ) ,
