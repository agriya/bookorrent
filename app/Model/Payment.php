...
class Payment extends AppModel
{
	...
	public function processUserSignupPayment($user_id, $payment_gateway_id = null)
    {
        ...
        if (empty($user['User']['is_paid'])) {
            ...
            if (Configure::read('user.is_email_verification_for_register') && empty($user['User']['is_openid_register']) && empty($user['User']['is_facebook_register']) && empty($user['User']['is_twitter_register']) && empty($user['User']['is_google_register']) && empty($user['User']['is_yahoo_register']) && empty($user['User']['is_linkedin_register']) && $user['User']['is_active'] == 0) {
