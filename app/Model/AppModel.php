...
class AppModel extends Model
{
    ...
    function findOrSaveAndGetId($data, $req)
    {
        ...
        if (!empty($findExist)) {
			...
			if(strtolower($this->name) == 'city' && !empty($req['state_id'])) {
				$this->data[$this->name]['state_id'] = $req['state_id'];
			}
			if(!empty($req['country_id'])){
				$this->data[$this->name]['country_id'] = $req['country_id'];			
			}
    ...
    function _sendAlertOnNewMessage($email, $message, $message_id, $template)
    {
        ...
        $this->User = new User();
        App::import('Model', 'Items.MessageContent');
    ...
    function _checkUserNotifications($to_user_id, $order_status_id, $is_sender, $is_contact = null)
    {
        ...
        $notification_check_array = array(
            '1' => 'is_new_item_order_host_notification',
            '2' => 'is_accept_item_order_booker_notification',
            '3' => 'is_reject_item_order_booker_notification',
            '4' => 'is_cancel_item_order_host_notification',
            '5' => 'is_arrival_host_notification',
            '6' => 'is_review_item_order_booker_notification',
            '7' => 'is_cleared_notification',
            '8' => 'is_complete_item_order_host_notification',
            '9' => 'is_expire_item_order_host_notification',
            '10' => 'is_cancel_item_order_host_notification',
            '11' => 'is_admin_cancel_item_order_host_notification',
            '12' => 'is_complete_later_item_order_host_notification',
        );
        $notification_check_sender_array = array(
            '1' => 'is_new_item_order_booker_notification',
            '2' => 'is_accept_item_order_host_notification',
            '3' => 'is_reject_item_order_host_notification',
            '4' => 'is_cancel_item_order_booker_notification',
            '5' => 'is_arrival_booker_notification',
            '6' => 'is_review_item_order_host_notification',
            '7' => 'is_cleared_notification',
            '8' => 'is_complete_item_order_booker_notification',
            '9' => 'is_expire_item_order_booker_notification',
            '10' => 'is_cancel_item_order_booker_notification',
            '11' => 'is_admin_cancel_booker_notification',
            '12' => 'is_complete_later_item_order_booker_notification',
    ...
    function getRatingCount($user_id)
    {
        ...
        $this->Item = new Item();
        $total_item_feedback_count = '';
        $total_item_positive_feedback_count = '';
        $get_user_total_ratings = $this->Item->find('all', array(
            'conditions' => array(
        ...
        foreach($get_user_total_ratings as $get_user_total_rating) {
            if (!empty($get_user_total_rating['Item']['item_feedback_count'])) {
                $total_item_feedback_count+= $get_user_total_rating['Item']['item_feedback_count'];
                $total_item_positive_feedback_count+= $get_user_total_rating['Item']['positive_feedback_count'];
            }
        }
        $getrating = $this->getItemRating($total_item_feedback_count, $total_item_positive_feedback_count);
        return $getrating;
    }
    function getItemRating($total_rating, $possitive_rating)
    {
        if (!$total_rating) {
    ...
    function getBookingChart($user_id = null, $item_id = null)
    {
        App::import('Model', 'Items.Item');
        $max_count = 0;
        $this->Item = new Item();
        if (empty($item_id)) {
            ...
            $chart_propery_ids = $this->Item->find('list', array(
                'conditions' => array(
                    'Item.user_id' => $user_id
                ) ,
                'fields' => array(
            ...
            if (!empty($chart_propery_ids)) {
                $conditions_propery_view['ItemView.item_id'] = $chart_propery_ids;
                $conditions_propery_user['ItemUser.item_id'] = $chart_propery_ids;
            } else {
                $conditions_propery_view['ItemView.item_id'] = 0;
                $conditions_propery_user['ItemUser.item_id'] = 0;
            }
        } else {
            $conditions_propery_view['ItemView.item_id'] = $item_id;
            $conditions_propery_user['ItemUser.item_id'] = $item_id;
        }
        $days = 3;
        for ($i = $days; $i > 0; $i--) {		
			 $conditions_propery_view["ItemView.created >= "] = date('Y-m-d 00:00:00', strtotime("-$i days"));
			 $conditions_propery_view["ItemView.created <= "] = date('Y-m-d 23:59:59', strtotime("-$i days"));
            $item_view_count = $this->Item->ItemView->find('count', array(
                'conditions' => $conditions_propery_view,
				'recursive' => -1,
            ));
            $chart_data['ItemView'][$i]['count'] = $item_view_count;
            if ($item_view_count > $max_count) {
                $max_count = $item_view_count;
            }
        }
        $days = 3;
        for ($i = $days; $i > 0; $i--) {
			 $conditions_propery_user["ItemUser.created >= "] = date('Y-m-d 00:00:00', strtotime("-$i days"));
			 $conditions_propery_user["ItemUser.created <= "] = date('Y-m-d 23:59:59', strtotime("-$i days"));
            $item_user_count = $this->Item->ItemUser->find('count', array(
                'conditions' => $conditions_propery_user,
				'recursive' => -1,
            ));
            $chart_data['ItemUser'][$i]['count'] = $item_user_count;
            if ($item_user_count > $max_count) {
                $max_count = $item_view_count;
            }
        }
	...
	public function getHtmlObj($viewObj){
