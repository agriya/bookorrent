...
<div class="userOpenids index js-response">
	...
	<div class="filter-section">
		<div>
			<?php echo $this->Form->input('q', array('label' => __l('Keyword'), 'maxlength' => '255')); ?>
		</div>
		<div>
    ...
    if (!empty($userOpenids)) :
       ...
       <div class="admin-select-block clearfix ">
            <?php echo __l('Select:'); ?>
            <?php echo $this->Html->link(__l('All'), '#', array('class' => 'js-select js-no-pjax {"checked":"js-checkbox-list"}')); ?>
            <?php echo $this->Html->link(__l('None'), '#', array('class' => 'js-select js-no-pjax {"unchecked":"js-checkbox-list"}')); ?>
        </div>
