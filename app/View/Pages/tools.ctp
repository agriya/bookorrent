...
<div class="pull-left sep-top space" id="js-confirm-message-block ">
	<div class="alert alert-info "><?php echo __l('When cron is not working, you may trigger it by clicking below link. For the processes that happen during a cron run, refer the ').$this->Html->link('product manual','http://dev1products.dev.agriya.com/doku.php?id=bookorrent-install#manual_cron_update_process', array('target'=>'_blank'));?></div>
