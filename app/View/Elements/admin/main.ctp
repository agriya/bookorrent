      <article id="<?php echo $this->Html->getUniquePageId();?>">
        <div class="container-fluid">        
 			<?php 
				...
				$items_menu=array('items', 'item_views', 'item_favorites','item_flags','collections','item_users','item_feedbacks','item_user_feedbacks');
				...
				$master_menu = array('currencies', 'email_templates','pages', 'transaction_types', 'translations', 'languages',  'banned_ips', 'cities', 'states', 'countries',  'user_educations', 'genders', 'user_employments', 'item_flag_categories', 'affiliate_widget_sizes', 'ips','cancellation_policies','request_flag_categories','user_relationships','user_income_ranges','habits');
                ...
                if(in_array($this->request->params['controller'], $user_menu) && $this->request->params['action'] != 'admin_diagnostics') {
					...
					$class = "diagnostics-title";
				} elseif($this->request->params['controller'] == 'halls' || $this->request->params['controller'] == 'partitions' || $this->request->params['controller'] == 'custom_price_per_types_seats') {
					$class = "icon-th";
				}
                ...
                echo $content_for_layout;
					...
					<h4 class="well space"><i class="<?php echo $class; ?> no-pad left-space text-16"></i> <span class="hor-smspace text-14 textb">					        
						...
						<?php if ($this->request->params['controller'] == 'settings' || $this->request->params['controller'] == 'payment_gateways' || $this->request->params['controller'] == 'extensions_plugins') { ?>
							<span class="setting-info info text-13 pull-right grayc hor-mspace "><?php echo sprintf(__l('To reflect changes, you need to %s'), $this->Html->link(__l('clear cache.'), array('controller' => 'devs', 'action' => 'clear_cache', '?f=' . $this->request->url), array('title' => __l('clear cache'), 'class' => 'js-delete mob-clr')));?></span>
						<?php } ?>
						</h4>
