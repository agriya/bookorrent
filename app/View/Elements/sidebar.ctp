<div class="pull-right dropdown"> <a href="#" title="<?php echo __l('Edit');?>" class="dropdown-toggle btn btn-large text-14 textb js-no-pjax graylighterc no-shad" data-toggle="dropdown"><i class="icon-cog graydarkc no-pad text-16"></i> <span class="caret"></span></a>
	<ul class="dropdown-menu arrow arrow-right">
		...
		<?php if (isPluginEnabled('Sudopay')){?>
		   ...
		   <li class="<?php echo $class;?>"><?php echo $this->Html->link('<i class="icon-money"></i>'.__l('Payout Preferences'), array('controller' => 'sudopays', 'action' => 'payout_connections'), array('title' => __l('Payout Preferences'),'escape'=>false));?></li>
		<?php }?>
