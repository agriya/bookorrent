      ...
      <tbody>
        ...
        if (!empty($blocks)):
          ...
          <tr>
            ...
            <td>
              <?php echo $this->Html->cText($block['Region']['title'], false);?>
            </td>
            <td class="dc">
              <?php echo $this->Html->link($this->Layout->status($block['Block']['status']), array('controller' => 'blocks', 'action' => 'update_status', $block['Block']['id'], 'status' => ($block['Block']['status'] == 1) ? 'deactivate' : 'activate'), array('class' => 'js-confirm js-no-pjax', 'title' => $block['Block']['title'], 'escape' => false));?>
            </td>
          </tr>
        <?php }
        else: ?>
          <tr>
            <td colspan="5"><div class="space dc grayc"><p class="ver-mspace top-space text-16 "> <?php echo __l('No Blocks available');?></p></div>
            </td>
          </tr>
        <?php
        endif;
