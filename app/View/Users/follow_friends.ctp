      ...
      <div>
        <?php foreach($followFriends as $followFriend) { ?>
          <div class="bot-space clearfix">
            ...
            <span class="pull-left ver-space"><a href="<?php echo Router::url(array('controller' => 'users', 'action' => 'view', $followFriend['User']['username'])); ?>" title="<?php echo $this->Html->cText($followFriend['User']['username'], false); ?>" class="blackc no-mar  dropdown-toggle js-no-pjax span show"><span class="pull-left  span"><?php echo $this->Html->getUserAvatar($followFriend['User'],  'micro_thumb',  0); ?></span><span class="span2 blackc hor-mspace htruncate"><?php echo $followFriend['User']['username']; ?></span></a></span>
          </div>
        <?php } ?>
      </div>
