...
<div class="clearfix sep-bot ver-mspace">
    <div class="ver-space ver-mspace clearfix register-social-icons">
      ...
      <?php if (Configure::read('twitter.is_enabled_twitter_connect')){ ?>
        ...
        <?php echo $this->Html->link('<i class="icon-twitter-sign twitterc text-24"></i><span class="hide">'.__l('Twitter').'</span>', '#', array('title' => __l('Twitter'), 'escape' => false, 'class' => "no-under js-connect js-no-pjax {'url':'$url'}")); ?>
        </li>
      <?php } ?>
      <?php if (Configure::read('linkedin.is_enabled_linkedin_connect')){ ?>
        ...
        <?php echo $this->Html->link('<i class="icon-linkedin-sign linkedc text-24"></i><span class="hide">'. __l('LinkedIn').'</span>', '#', array('title' => __l('LinkedIn'), 'escape' => false, 'class' => "no-under js-connect js-no-pjax {'url':'$url'}")); ?>
        </li>
      <?php } ?>
      <?php if (Configure::read('yahoo.is_enabled_yahoo_connect')){ ?>
        ...
        <?php echo $this->Html->link('<i class="icon-yahoo yahooc text-24"></i><span class="hide">'.__l('Yahoo!').'</span>', '#', array('title' => __l('Yahoo!'), 'escape' => false, 'class' => "no-under js-connect js-no-pjax {'url':'$url'}")); ?>
        </li>
      <?php } ?>
      <?php if (Configure::read('google.is_enabled_google_connect')) { ?>
        ...
        <?php echo $this->Html->link('<i class="icon-google-sign googlec text-24"></i><span class="hide">'.__l('Google').'</span>', '#', array('title' => __l('Google'), 'escape' => false,'class' => "no-under js-connect js-no-pjax {'url':'$url'}")); ?>
        </li>
      <?php } ?>
	  <?php if (Configure::read('googleplus.is_enabled_googleplus_connect')) { ?>
        ...
        <?php echo $this->Html->link('<i class="icon-google-plus-sign googleplusc text-24"></i><span class="hide">'.__l('Google+').'</span>', '#', array('title' => __l('Google+'), 'escape' => false,'class' => "no-under js-connect js-no-pjax {'url':'$url'}")); ?>
        </li>
      <?php } ?>
      <?php if (Configure::read('openid.is_enabled_openid_connect')) { ?>
        ...
        <?php echo $this->Html->link(__l('OpenID'), '#', array('title' => __l('OpenID'), 'escape' => false,'class' => "no-under show js-connect js-no-pjax {'url':'$url'}")); ?>
        </li>
      <?php } ?>
       </ul>
     </div>
...
<?php endif; ?>
	...
	<?php } else { ?>
			...
			if(empty($this->request->data['User']['openid_url']) && empty($this->request->data['User']['facebook_user_id']) && empty($this->request->data['User']['twitter_user_id']) && empty($this->request->data['User']['is_yahoo_register']) && empty($this->request->data['User']['is_google_register']) && empty($this->request->data['User']['is_googleplus_register']) && empty($this->request->data['User']['is_linkedin_register']) && empty($this->request->data['User']['is_openid_register'])):
				if(isPluginEnabled('SecurityQuestions')) {
					...
					echo $this->Form->input('security_question_id',array('id'=>'js-security_question_id', 'empty' => __l('Please select questions'), 'label' => __l('Security Questions')));
					echo '</div>';
	...
	?>
	<div class="mob-no-mar input checkbox terms_error required">
	<?php echo $this->Form->input('is_agree_terms_conditions', array('div' => false, 'type' => 'checkbox','label' => __l('I have read, understood & agree to the').' '.$terms, 'class' => 'js-remove-error no-mar')); ?>
	</div>
