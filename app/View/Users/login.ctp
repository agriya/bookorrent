...
<section class="thumbnail clearfix img-rounded">
  ...
  <div class="row ver-space sep-bot">
    <div class="span5 login-block pr no-mar">
    <?php if (isPluginEnabled('SocialMarketing')) { ?>
    <span class="js-facepile-loader loader pull-left offset1 pa space"></span>
    <span id="js-facepile-section" data-fb_app_id="<?php echo Configure::read('facebook.app_id'); ?>" class="sfont"></span>
    <?php } ?>
    &nbsp;
    </div>
  ...
  <div class="span7 offset2 bot-space">
 ...
 <?php endif;?>
  ...
  ?>
  <h4 class="span ver-space top-mspace pr"><span class="hidden-sm or-ver pa textb"><?php echo __l('Or');?></span></h4>
  <?php } else {
  ...
  echo $this->Form->input(Configure::read('user.using_to_login'), array('class' => 'span6','label' => __l('Username')));
  ...
  <?php if (!(!empty($this->request->params['prefix']) && $this->request->params['prefix'] == 'admin') && empty($this->request->params['requested'])):  ?> | <?php echo $this->Html->link(__l('Sign Up'), array('controller' => 'users', 'action' => 'register','type'=>'social', 'admin' => false), array('title' => __l('Sign Up')));?><?php endif; ?>
  <?php
  ...
  if(!$thirdParty) {
	$f = 'items/order/'.$this->request->params['named']['payment_id'].'/order_id:'.$this->request->params['named']['order_id'];
  ...
  <div class="top-space submit-block clearfix">
