...
<?php endif; ?>			  
			<div class="clearfix dc">
					...
					<?php echo $this->Html->link('<span class="top-smspace no-pad icon-plus-sign"></span>', array('controller' => 'users', 'action' => 'add'), array('escape' => false,'class' => 'btn right-mspace btn-primary textb text-18','title'=>__l('Add'))); ?>					
...
<?php echo $this->Form->input('r', array('type' => 'hidden', 'value' => $this->request->url)); ?>
 <table class="table list no-round" id="js-expand-table">
	...
	<tr class=" well no-mar no-pad js-even">
        ...
        <th colspan="2" class="dc graydarkc sep-bot sep-right "><?php echo __l('As') . ' ' . Configure::read('item.alt_name_for_booker_singular_caps');?></th>
 	...
 	<tr class="well no-mar no-pad js-even">
        ...
        <th class="dc sep-right graydarkc"><?php echo Configure::read('item.alt_name_for_item_plural_caps');?></th>
        <th class="dr sep-right graydarkc"><?php echo __l('Site Revenue').' ('.Configure::read('site.currency').')';?></th>
...
<tbody>
     <tr class="<?php echo $class.' '.$active_class;?> expand-row js-odd">
     	<td class="<?php echo $class;?>"><?php echo $this->Form->input('User.'.$user['User']['id'].'.id', array('type' => 'checkbox', 'id' => "admin_checkbox_".$user['User']['id'], 'label' => "", 'before' => '<span class="show pull-left hor-smspace"><i class="icon-caret-down"></i></span>', 'class' => $status_class.' js-checkbox-list')); ?></td>     	
		<td class="span5 no-mar">
			...
			<div class="clearfix">
				...
				<?php if(!empty($user['User']['email'])):?>
					...
					<span class="pull-left" title="<?php echo $this->Html->cText($user['User']['email'], false); ?>">
				<?php
       ...
       <td class="dc"><?php echo $this->Html->cInt($user['User']['booking_total_booked_count']); ?></td>
       <td class="dr site-amount"><?php echo $this->Html->cCurrency($user['User']['booking_total_site_revenue']); ?></td>
       <td class="dc"><?php echo $this->Html->cInt($user['User']['item_count']); ?></td>
       ...
       <td class="dc"><?php echo ($user['User']['last_logged_in_time']=='0000-00-00 00:00:00' || empty($user['User']['last_logged_in_time']))? '-': $this->Html->cDateTimeHighlight($user['User']['last_logged_in_time']);?></td>
       <td class="dl">
		<?php if(!empty($user['LastLoginIp']['ip'])): ?>
			<?php echo  $this->Html->link($user['LastLoginIp']['ip'], array('controller' => 'users', 'action' => 'whois', $user['LastLoginIp']['ip'], 'admin' => false), array('target' => '_blank', 'class'=>'grayc js-no-pjax', 'title' => __l('whois') .' '.$user['LastLoginIp']['host'], 'escape' => false));
			?>
			<?php
				if(!empty($user['LastLoginIp']['Country'])):
			?>
					<div><span class="flags flag-<?php echo strtolower($user['LastLoginIp']['Country']['iso_alpha2']); ?>" title ="<?php echo $this->Html->cText($user['LastLoginIp']['Country']['name'], false); ?>"><?php echo $this->Html->cText($user['LastLoginIp']['Country']['name'], false);  ?></span> <p class="inline span2 htruncate"><?php if(!empty($user['LastLoginIp']['City'])): echo $this->Html->cText($user['LastLoginIp']['City']['name'], false);  endif; ?> </p></div>
			<?php endif; ?>
			<?php else: ?>
				<?php echo __l('N/A'); ?>
			<?php endif; ?>
		...
		<td class="dl">
			<?php
			if(!empty($user['Ip']['ip'])):
					echo  $this->Html->link($user['Ip']['ip'], array('controller' => 'users', 'action' => 'whois', $user['Ip']['ip'], 'admin' => false), array('target' => '_blank', 'class'=>'grayc js-no-pjax',  'title' => __l('whois').' '.$user['Ip']['ip'], 'escape' => false));
			?>
			<?php
				if(!empty($user['Ip']['Country'])):
			?>
					<div><span class="flags flag-<?php echo strtolower($user['Ip']['Country']['iso_alpha2']); ?>" title ="<?php echo $this->Html->cText($user['Ip']['Country']['name'], false); ?>"><?php echo $this->Html->cText($user['Ip']['Country']['name'], false);  ?></span> <p class="inline span2 htruncate"><?php if(!empty($user['Ip']['City'])): echo $this->Html->cText($user['Ip']['City']['name'], false);  endif; ?></p></div>
			<?php endif; ?>
			<?php else: ?>
				<?php echo __l('n/a'); ?>
			<?php endif; ?>
                      ...
                      <tr class="hide sep-bot sep-medium">
                        <td colspan="11"><div class="top-space clearfix">
                            <div class="clearfix activities-block">
                              <div class="pull-right dropdown"> <a data-toggle="dropdown" class="dropdown-toggle btn btn-large text-14 textb graylighterc no-shad" title="<?php echo __l('Edit');?>" href="#"><i class="icon-cog graydarkc  no-pad text-16"></i> <span class="caret"></span></a>
                            ...
                            <li><?php echo $this->Html->link('<i class="icon-remove"></i>'.__l('Delete'), array('action'=>'delete', $user['User']['id']), array('escape' => false,'class' => 'delete js-delete graydarkc', 'title' => __l('Delete')));?></li>
                             ...
                             <li>
                              ...
                              <ul id="myTab3" class="nav nav-tabs top-smspace">
                                <li class="active"><a href="#As-Host-<?php echo $this->Html->cInt($user['User']['id'], false); ?>" data-toggle="tab"><?php echo __l('As Host'); ?></a> </li>
                                <li><a href="#As-Booker-<?php echo $this->Html->cInt($user['User']['id'], false); ?>" data-toggle="tab"
><?php echo __l('As') . ' ' . Configure::read('item.alt_name_for_booker_singular_caps'); ?></a></li>
                                <li><a  data-toggle="tab" href="#As-User-<?php echo $this->Html->cInt($user['User']['id'], false); ?>"><?php echo __l('As User'); ?></a></li>
                              </ul>
                              <div class="sep-right sep-left sep-bot tab-round tab-content" id="myTabContent3">
                                <div class="tab-pane space active" id="As-Host-<?php echo $this->Html->cInt($user['User']['id'], false); ?>">
                                    ...
                                    <div class="pull-right">
                                      <div class="span8">
										...
										<dl class="list">
											...
											<dd title="<?php echo $user['User']['item_pending_approval_count']; ?>" class="textb text-16 graydarkc pr hor-mspace"><?php echo $this->Html->cInt($user['User']['item_pending_approval_count']); ?></dd>
										</dl>
										<dl class="list">
										   <dt class="pr hor-mspace text-11"><?php echo __l('Enabled'); ?></dt>
										   <dd title="<?php echo $user['User']['item_count']; ?>" class="textb text-16 graydarkc pr hor-mspace"><?php echo $this->Html->cInt($user['User']['item_count']); ?></dd>
									   </dl>
									   <dl class="list">
										   <dt class="pr hor-mspace text-11"><?php echo __l('Disabled'); ?></dt>
										   <dd title="<?php echo $user['User']['item_inactive_count'];?>" class="textb text-16 graydarkc pr hor-mspace"><?php echo $this->Html->cInt($user['User']['item_inactive_count']); ?></dd>
									   </dl>
                                        </div>
                                      </div>
                                    </div>
                                    ...
                                    <div class="pull-right">
                                      <div class="span8">
										...
										<dl class="list">
											...
											<?php echo numbers_to_higher($user['User']['item_feedback_count'] - $user['User']['positive_feedback_count']); ?>
											</dd>
										</dl>
										<dl class="list">
											<?php
											$success_rate = $user['User']['item_feedback_count'] - $user['User']['positive_feedback_count'];
											?>
											<dt  class="pr hor-mspace text-11" title ="<?php echo __l('Success Rate');?>"><?php echo __l('Success Rate');?></dt>
											<?php if(empty($user['User']['item_feedback_count'])): ?>
                                ...
                                <div id="As-Booker-<?php echo $this->Html->cInt($user['User']['id'], false); ?>" class="tab-pane space" >
                                  <div class="row no-mar">
                                    <div class="pull-left">
                                      <div class="span8">
                                        <h3 class="well space textb text-16 no-mar"><?php echo __l('Amount'); ?></h3>
                                        <div class="clearfix ver-space bot-mspace">
										   <dl class="list">
											   ...
											   <dd class="textb text-16 graydarkc pr hor-mspace"><?php echo $this->Html->siteCurrencyFormat($user['User']['booking_total_booked_amount']); ?></dd>
										   </dl>
										   <dl class="sep-left list">
											   <dt class="pr hor-mspace text-11"><?php echo __l('Site Revenue'); ?></dt>
											   <dd class="textb text-16 graydarkc pr hor-mspace"><?php echo $this->Html->siteCurrencyFormat($user['User']['booking_total_site_revenue']); ?></dd>
											</dl>						
                                        </div>
                                      </div>
                                    </div>
                                    <div class="pull-right">
                                      <div class="span8">
                                        ...
                                        <div class="clearfix ver-space bot-mspace">
											<dl class="list">
        								   <dt class="pr hor-mspace text-11"><?php echo __l('Successful'); ?></dt>
										   <dd class="textb text-16 graydarkc pr hor-mspace"><?php echo $this->Html->cInt($user['User']['booking_total_booked_count']); ?></dd>
										   </dl>
										   <dl class="sep-left list">
        								   <dt class="pr hor-mspace text-11"><?php echo __l('Unsuccessful'); ?></dt>
										   <dd class="textb text-16 graydarkc pr hor-mspace"><?php echo $this->Html->cInt($user['User']['booking_total_lost_booked_count']); ?></dd>
										   </dl>
                                        </div>
                                      </div>
                                    </div>
                                      ...
                                      <div class="span8">
                                        ...
                                        <div class="clearfix ver-space bot-mspace">
											<dl class="list">										
												...
												<dd class="textb text-16 graydarkc pr hor-mspace">
													<?php echo numbers_to_higher($user['User']['booker_positive_feedback_count']); ?>
												</dd>
                                            </dl>
                                            <dl class="list">
												<dt class="pr hor-smspace text-11" title ="<?php echo __l('Negative');?>"><?php echo __l('Negative');?></dt>
												<dd class="textb text-16 graydarkc pr hor-mspace"> <?php echo numbers_to_higher($user['User']['booker_item_user_count'] - $user['User']['booker_positive_feedback_count']);?>	</dd>
                                            </dl>
                                ...
                                <div id="As-User-<?php echo $this->Html->cInt($user['User']['id'], false); ?>" class="tab-pane space">
                                  <div class="row no-mar">
                                    <div class="span12 right-space">
                                      <div class="clearfix ver-space bot-mspace">
                                        ...
                                        <dl class="list">
										   ...
										   <dd class="textb text-16 graydarkc pr hor-mspace"><?php echo $this->Html->siteCurrencyFormat($user['User']['booking_total_site_revenue'] + $user['User']['host_total_site_revenue']); ?></dd>
										 </dl>
                                      </div>
                                    </div>
                                  </div>
                            ...
                            <div class="clearfix details-block no-mar pull-right">
                              ...
                              <div class="pull-left hor-space bot-mspace "> <?php echo $this->Html->link($this->Html->cText($user['User']['username'], false), array('controller' => 'users', 'action' => 'view', $user['User']['username'], 'admin' => false),array('title' => $user['User']['username'], 'escape' => false, 'class' => 'graydarkc text-24 textb mob-text-24 bot-mspace show js-no-pjax')); ?>	  
                                <dl class="no-mar clearfix">
									<dt class="pull-left textn no-mar"> <?php echo __l('Registered on'); ?></dt>
