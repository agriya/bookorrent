...
<html lang="<?php echo isset($_COOKIE['CakeCookie']['user_language']) ?  strtolower($_COOKIE['CakeCookie']['user_language']) : strtolower(Configure::read('site.language')); ?>">
  <head>
	...
	<!--[if IE 7]>
		<?php echo $this->Html->css('theme/pink/font-awesome-ie7.css', null, array('inline' => true, "media" => "all")); ?>
	<![endif]-->
    <?php
	  $lang = isset($_COOKIE['CakeCookie']['user_language']) ?  $_COOKIE['CakeCookie']['user_language'] : Configure::read('site.language');
      echo $this->Html->css('maintenance.cache.'.Configure::read('site.version'), null, array('inline' => true, "media" => "all"));
      $cms = $this->Layout->js();
    ...
    <?php } ?>
    <?php if (!empty($meta_for_layout['item_image'])) { ?>
