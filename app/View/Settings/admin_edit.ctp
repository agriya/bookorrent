<ul class="breadcrumb top-mspace ver-space">
		...
		<?php }elseif($this->request->params['controller'] == 'settings' && $this->request->params['action'] == 'admin_edit' ) { ?>
			<li><?php echo $this->Html->link(__l('Settings'), array('controller' => 'settings', 'action' => 'index'), array('title' => __l('Back to Settings')));?> <span class="divider">/</span></li> <li class="active"> <?php echo $this->Html->cText(__l($setting_categories['SettingCategory']['name']), false); ?> </li>					
		...
		<div class="alert alert-info">
			<?php
				if(stristr($setting_categories['SettingCategory']['description'], '##PAYMENT_SETTINGS_URL##') === FALSE) {
					echo $this->Html->cHtml($setting_categories['SettingCategory']['description'], false);
				} else {
		...
		if (!empty($settings)):
			...
			foreach ($settings as $key=>$setting):
				...
				if($options['type'] == 'select'):
					$selectOptions = explode(',', trim($setting['Setting']['options']));
			...
			<?php	if(!empty($is_changed)): 
					...
					if(in_array($setting['SettingCategory']['id'], array(69, 70, 71, 72, 73, 74))){ echo '</div></div></div>';}
?>					
					...
					<div class="<?php echo (in_array( $setting['SettingCategory']['id'], array(67,69,70))) ? 'span23 offset1' : '' ;?>">
					...
					<fieldset class="form-block">
						...
						'##DEMO_URL##' => $this->Html->link('http://dev1products.dev.agriya.com/doku.php?id=bookorrent-install#how_to','http://dev1products.dev.agriya.com/doku.php?id=bookorrent-install#how_to', array('target' => '_blank')),
										);
										$setting['SettingCategory']['description'] = strtr($setting['SettingCategory']['description'], $findReplace);
										echo $this->Html->cHtml($setting['SettingCategory']['description'], false);
									?>
								</div>
							<?php endif;?>
						</fieldset>
			...
			<?php 	endif; ?> 		
				<?php if(!empty($is_changed)) { ?>                           
                        <?php if ($setting['SettingCategory']['id'] == 67) { ?>
						 <div class="span10 well pull-right">
                            ...
                            <?php echo __l('2. To create token please refer '); ?> <a target="_blank" href="http://blog.cloudflare.com/2-factor-authentication-now-available">http://blog.cloudflare.com/2-factor-authentication-now-available</a><br>
                            <?php echo __l('3. Create three page rules like /, /item/*, /user/* in this link'); ?> <a target="blank" href="https://www.cloudflare.com/page-rules?z=<?php echo $_SERVER["SERVER_NAME"]; ?>">https://www.cloudflare.com/page-rules?z=<?php echo $_SERVER["SERVER_NAME"]; ?></a><?php echo __l('. Note: Please select \'Cache Everything\' option for \'Custom Caching\' setting.'); ?><br>
						...
						<div class="span10 well pull-right">
                            <h4><?php echo __l('Configuration steps:');?></h4> <br>
                            <?php echo __l('1. Amazon CloudFront: To setup Amazon CloudFront CDN please follow the step mentioned in this <a target="_blank" href="http://aws.amazon.com/console/#cf">http://aws.amazon.com/console/#cf</a> and watch this screencast <a href="http://d36cz9buwru1tt.cloudfront.net/videos/console/cloudfront_console_4.html" target="blank">http://d36cz9buwru1tt.cloudfront.net/videos/console/cloudfront_console_4.html</a>'); ?><br>
                            <?php echo __l('2. CloudFlare: To setup CloudFlare please follow the step mentioned in this link <a href="https://support.cloudflare.com/entries/22054357-How-do-I-do-CNAME-setup-" target="_blank">https://support.cloudflare.com/entries/22054357-How-do-I-do-CNAME-setup-</a>'); ?><br>
                        </div>
						...
						<div class="span10 well pull-right">
                            ...
                            <?php echo __l('1. Amazon SES: To get your security credentials, login with amazon and go to <a target="_blank" href="https://portal.aws.amazon.com/gp/aws/securityCredentials#access_credentials">https://portal.aws.amazon.com/gp/aws/securityCredentials#access_credentials</a> . To create your smtp username password go to <a target="blank" href="https://console.aws.amazon.com/ses/home#smtp-settings">https://console.aws.amazon.com/ses/home#smtp-settings</a>'); ?><br>
                            <?php echo __l('2. Sendgrid: To get your security credentials, refer <a target="_blank" href="http://sendgrid.com/docs/Integrate/index.html">http://sendgrid.com/docs/Integrate/index.html</a>'); ?><br>
                            <?php echo __l('3. Mandrill:  To get your security credentials, login with Mandrill and go to <a target="_blank" href="https://mandrillapp.com/settings">https://mandrillapp.com/settings</a>'); ?><br>
                            <?php echo __l('4. Gmail: To use gmail please refer <a target="_blank" href="http://gmailsmtpsettings.com/gmail-smtp-settings">http://gmailsmtpsettings.com/gmail-smtp-settings</a>'); ?>
                         </div>
					   <?php }  if ($setting['SettingCategory']['id'] == 74) { ?>
						<div class="span10 well pull-right">
							<h4><?php echo __l('Configuration steps:'); ?></h4><br>
							<?php echo __l('1. To get your security credentials, login with amazon and go to <a target="_blank" href="https://portal.aws.amazon.com/gp/aws/securityCredentials#access_credentials">https://portal.aws.amazon.com/gp/aws/securityCredentials#access_credentials</a><br>2. To create bucket name go to <a target="_blank" href="https://console.aws.amazon.com/s3/home">https://console.aws.amazon.com/s3/home</a> and click s3 link.'); ?>
						</div>
				<?php } ?> 
  	...
  	<?php echo $this->Html->link('<span><i class="icon-google-sign googlec space text-16"></i>' . __l('Update Google Analytics Credentials') . '</span>', array('controller' => 'settings', 'action' => 'update_credentials', 'type' => 'google'), array('class' => 'btn tp-credential js-bootstrap-tooltip', 'escape' => false, 'title' => __l('Here you can update Google Analytics credentials like Access Token. Click this link and Follow the steps. Please make sure that you have updated the Consumer Key and Consumer secret before you click this link.'))); ?>
				...
				if ($setting['Setting']['name'] == 'cdn.images' || $setting['Setting']['name'] == 'cdn.js' || $setting['Setting']['name'] == 'cdn.css'){	
					...
					if (in_array( $setting['Setting']['name'], array('wallet.min_wallet_amount', 'wallet.max_wallet_amount', 'user.minimum_withdraw_amount', 'user.maximum_withdraw_amount','user.signup_fee', 'affiliate.payment_threshold_for_threshold_limit_reach','item.booking_service_fee','item.host_commission_amount','item.item_fee','item.verify_fee'))):
						if($setting['Setting']['name']=='item.host_commission_amount'){
						$options['after'] = '%' . '<span class="info">' . $setting['Setting']['description'] . '</span>';
						}
						else{
					...
					if($setting['Setting']['name'] == 'Site.logo'):
						 $options['after'] = '<div class="settings-site-logo">'.$this->Html->showImage('SiteLogo', $attachment['SiteLogo'], array('full_url' => true,'dimension' => 'site_logo_thumb', 'alt' => __l('[Image: SiteLogo]'), 'title' =>  __l('SiteLogo'), 'type' => 'png', 'class' => 'siteLogo')).'</div>';
					endif;
                  ...
                  <div class="show"> 
						<?php 
						if (in_array( $setting['Setting']['id'], array(206, 304, 265, 264, 319, 373,443))){
