		...
		<div class="profile-image">
                ...
                <?php
					$current_user_details = array(
						...
						'facebook_user_id' => $this->request->data['User']['facebook_user_id']
					);
					$current_user_details['UserAvatar'] = array(
						'id' => $this->request->data['User']['attachment_id']
					);
					echo $this->Html->getUserAvatar($current_user_details, 'big_thumb', true);
				?>
				<?php if(isPluginEnabled('SocialMarketing')) {
						echo $this->Html->link(__l('Change Image'), array('controller' => 'user_profiles', 'action' => 'profile_image',$this->request->data['User']['id'], 'admin' => false), array('class' => 'show dc')); 
					}
				?>
    		</div>
	   </div>
	   <div class="span17 top-mspace pull-left space form-align"> 
    		...
    		<?php
    		   $required="required";
    			...
    			if($this->request->data['User']['role_id'] == ConstUserTypes::Admin):
    				echo $this->Form->input('User.username',array('readonly' => 'readonly','label'=>__l('Username')));
    			endif;
    			if($this->Auth->user('role_id') == ConstUserTypes::Admin):
			...
			?>
				<div class="show bot-mspace profile-gender-block <?php echo (empty($this->request->params['prefix']))?'required':''; ?>">
					<label>	<?php echo __l('Gender'); ?> </label>
								<?php echo $this->Form->input('gender_id', array('type' => 'radio', 'legend' => false,'div'=> array('class' => 'input radio no-mar'), 'id' => 'js-gender')); ?>
					</div>
					...
					<?php echo $this->Form->input('user_income_range_id', array('empty' => __l('Please Select'),'label' => __l('Income range').'('.configure::read('site.currency').')')); ?>
					<?php $options = array('1' => __l('Yes'), '0' => __l('No')); ?>
					<div class="show bot-mspace">
						<label><?php echo __l('Own Home?'); ?></label>
					...
					<div class="show bot-mspace">
						<label><?php echo __l('Have Children?'); ?></label>
					...
					<?php if(isPluginEnabled('Items')){ ?>
						...
						<p class="add-info addone-info pull-left"><?php echo __l('Habit'); ?></p>
						<?php
						echo $this->Form->input('Habit', array('type'=>'select', 'multiple'=>'checkbox', 'class'=> 'checkbox no-mar top-smspace', 'id'=>'Habit1', 'label' =>false));
						?>
						</div>					
                    <?php
				    }
    		...
    		?>
				<?php 
					 if(!isPluginEnabled('SocialMarketing')) {
						...
						echo $this->Form->input('UserAvatar.filename', array('type' => 'file','size' => '33', 'label' => __l('Upload Photo'),'class' =>'browse-field height-field'));
					}
				?>
			</div>
    	</fieldset>
       <fieldset>        
			<h3 class="well space textb text-16"><?php echo __l('Contact details'); ?></h3>
				...
				<?php
					...
					echo $this->Form->input('country_id',array('id'=>'js-country_id', 'label' => __l('Country'), 'empty' => __l('Please Select')));
					echo $this->Form->input('zip_code', array('label' => __l('Zip code'), 'id' => 'ItemPostalCode'));
    	  ...
    	  <fieldset>
			...
			<fieldset>
			<h3 class="well space textb text-16"><?php echo __l('Message'); ?></h3>
    		<?php
    			echo $this->Form->input('message_signature', array('label' => __l('Message Signature'), 'type' => 'textarea'));
    		?> 
			</fieldset>
