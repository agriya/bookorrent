	...
	<div id="paymentgateways-tab-container" class="">
		<ul class="nav nav-tabs no-mar">
			<?php foreach($gateway_groups As $gateway_group) { ?>
				<li><a href="#paymentGateway-<?php echo $gateway_group['id']; ?>" class="js-no-pjax" data-toggle="tab"><div class="pull-left starts-left">
				<?php if(!empty($gateway_group['thumb_url'])){ ?>
				<img src="<?php echo $gateway_group['thumb_url']; ?>" alt="<?php echo $this->Html->cHtml($gateway_group['display_name'], false); ?>" />
				<?php } ?>
				<span class="show"><?php echo $this->Html->cHtml($gateway_group['display_name'], false); ?></span></div></a></li>
			<?php } ?>
		</ul>
					...
					if($payment_gateway['group_id'] == $gateway_group['id']) {
						if ($payment_gateway['payment_id'] == ConstPaymentGateways::Wallet) {
							...
							if ($payment_gateway['group_id'] == 4922):
								$option_value = '<div class="pull-left starts-left"><span class="show">' . __l('Credit & Debit Cards') . '</span></div>';
							else:
								$option_value = '<div class="pull-left"><img src="'. $payment_gateway['thumb_url'] .'" alt="'.$payment_gateway['display_name'].'"/><span class="show">'.$payment_gateway['display_name'].'</span></div>';
...
?>
<div class="js-instruction js-instruction_<?php echo $key; ?> alert alert-info top-mspace hide">
...
<?php if (!empty($gateways_response->form_fields_tpls)) { ?>
	<div class="js-form">
		<?php foreach($gateways_response->form_fields_tpls as $key => $value) { ?>
			<div class="js-gatway_form_tpl hide gateway-form-tpl" id="form_tpl_<?php echo $key; ?>">
				<?php if($key == 'buyer'){ ?>
				...
				<?php if($key == 'credit_card'){ ?>
					...
					<?php echo $this->Html->image('credit-detail.png', array('alt'=>'[Image: Credit Card]'));?>
					</span></h3>
				<?php } ?>
				...
				<?php
					foreach($value['_fields'] as $field_name => $required) {
						...
						if (!empty($required)) {
							$input_class .= " required validation:{'rule1':{'rule':'notempty','message':'Required'}}";
						}
						...
						?>
							<h3 class="well space textb text-16 ver-mspace"><?php echo __l('Payment Details'); ?></h3>
						<?php
							$type = 'textarea';
						}
...
<div class="submit-block form-payment-panel clearfix">
	<div class="submit">
		...
		<p class="top-space bot-sp textb available-balance js-user-available-balance {'balance':'<?php echo $logged_in_user['User']['available_wallet_amount']; ?>'}">
		<?php echo __l('Your available balance:').' '. $this->Html->siteCurrencyFormat($this->Html->cCurrency($logged_in_user['User']['available_wallet_amount'], false));?>
	</p>
			<?php
			echo $this->Form->submit(__l('Pay with Wallet'), array('name' => 'data['.$model.'][wallet]', 'class' => '{"balance":"' . $logged_in_user['User']['available_wallet_amount'] . '"}  btn btn-large wallet-button ' . ' js-update-order-field js-no-pjax', 'div' => false));
			?>
		</div>
		<div class= "js-normal-sudopay hide">
			<?php
				echo $this->Form->submit(__l('Pay Now'), array('name' => 'data['.$model.'][wallet]', 'div' => false, 'id' => 'sudopay_button', 'class' => 'btn btn-large'));
		?>
		</div>
	</div>   
</div>