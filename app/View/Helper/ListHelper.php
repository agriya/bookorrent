...
class ListHelper extends AppHelper
{
    ...
    private $_defaultSettings = array(
        ...
        'alias' => 'title',
        'primaryKey' => 'id',
        'emptyMessage' => __l('There are no records to display.')
    );
    private $_emptyMessage = __l("There are no records to display.");
    ...
    function create($data, $settings = array()) 
    {
        ...
        if (empty($settings['model'])) {
            trigger_error(__l('Model class name can not be empty!'));
        }
        // Start the list
        $html = "\n<ul";
        ...
        foreach($data as $key => $node) {
            ...
            if (isset($settings['element'])) {
                ...
                $html.= $view->element($settings['element'], array(
                    'node' => $node
                ));
                ...
                $html.= $this->Html->link($node[$settings['model']][$settings['alias']], array(
                    'action' => 'edit',
                    'id' => $node[$settings['model']][$settings['primaryKey']]
                ));
            }
            $html.= "</li>\n";
        }
    ...
    function createNestedTree($data, $settings = array()) 
    {
        ...
        foreach($data as $i => $node) {
            ...
            if ($node[$model][$right] <> $node[$model][$left]+1) { // Has some children
                ...
                $last = !empty($parentRight == ($node[$model][$right]+1) ) ? true : false;
                $this->_limits[] = array(
                    $node[$model][$left],
                    $node[$model][$right]
                );
            ...
            if (function_exists('treeItemCallback')) {
                ...
                $return.= $this->Html->link($node[$model][$alias], array(
                    'action' => 'edit',
                ...
                if ($node[$model][$right] < $data[$i+1][$model][$left]) { // Close uls
                    for ($j = 1; $j <= ($data[$i+1][$model][$left]-$node[$model][$right]-1); $j++) {
                        ...
                        if ($this->_childCount < 0) {
                            trigger_error(__l('child count less than 0 in ' . __METHOD__, E_USER_WARNING));
                        }
                        $return.= '</li>';
    ...
    function createTree($data, $settings = array()) 
    {
        ...
        foreach($data as $i => $node) {
            ...
            if (function_exists('listItemCallback')) {
                ...
                $return.= $this->Html->link($node[$settings['model']][$alias], array(
                    'action' => 'edit',
                    'id' => $node[$settings['model']][$settings['primaryKey']]
                ));
            }
