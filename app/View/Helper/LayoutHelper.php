...
class LayoutHelper extends AppHelper
{
    ...
    public function js()
    {
        ...
        $cms['cfg'] = array(
            ...
            'listing' => Configure::read('item.alt_name_for_item_singular_caps') ,
            'timezone' => date('Z') /(60*60) ,
            'date_format' => 'M d, Y',
            'today_date' => date('Y-m-d'),
			'Available' => __l('Available'),
			'Unavailable' => __l('Unavailable'),
			'Blocked' => __l('Blocked'),
			'Booking' => __l('Booking'),
			'Booked' => __l('Booked'),
			'Selected' => __l('Selected'),
			'No seats' => __l('No seats'),
			...
			'week_start_day'	=> date('w', strtotime(Configure::read('item.weekstartson'))),
			'calendar_type'		=> Configure::read('item.set_default_calendar_type'),
			'site_lang' => isset($_COOKIE['CakeCookie']['user_language']) ?  $_COOKIE['CakeCookie']['user_language'] : Configure::read('site.language'),
	...
	function adminMenus($menus, $options = array())
    {
		...
		$out = '<div class="navbar"><div class="navbar-inner hor-space bot-shad no-bor no-round sep-bot clearfix"><a class="btn btn-navbar ver-smspace" data-toggle="collapse" data-target=".nav-collapse"> <i class="icon-align-justify icon-24 no-pad whitec"></i></a><div class="nav-collapse">';
        $out.= '<div class="container-fluid "><ul class="nav admin-menu tab-no-pad">';
        $icon_class = '';
		$intHeader = 0;
		$intSubMenu = 1;
        ...
        foreach($sorted as $menu) {
            ...
            if($menu['icon-class'] == 'admin-payment' && ($this->request->params['controller'] == 'pledges' || $this->request->params['controller'] == 'donates' || $this->request->params['controller'] == 'lends' || $this->request->params['controller'] == 'invests') && $this->request->params['action'] == 'admin_funds') {
                $payment_active = 'active';
                $item_active = '';
                $out.= '<li class="no-mar dc dropdown mob-clr sep-none ' . $payment_active .  $class .' ">';
            } else if($menu['icon-class'] == 'file' && ($this->request->params['controller'] == 'pledges' || $this->request->params['controller'] == 'donates' || $this->request->params['controller'] == 'lends' || $this->request->params['controller'] == 'invests') && $this->request->params['action'] == 'admin_index') {
                $item_active = 'active';
                $payment_active = '';
                $out.= '<li class="no-mar dc dropdown mob-clr sep-none ' . $item_active .  $class .' ">';
                ...
                $out.= '<li class="no-mar dc dropdown mob-clr sep-none ' . $payment_active .  $class .' ">';
                   ...
                   if($menu['title'] == 'Payments' && ($this->request->params['controller'] == 'pledges' || $this->request->params['controller'] == 'donates' || $this->request->params['controller'] == 'lends' || $this->request->params['controller'] == 'invests' || $this->request->params['controller'] == 'settings')) {
                        $active_class = '';
                    }
                    if($menu['title'] == 'Items') {
                        $active_class = '';
					...
					if($this->request->params['controller'] == 'categories' && $menu['icon-class'] == 'random')
					{
						$active_class = 'active';
					}
                $out.= '<li class="no-mar dc dropdown mob-clr sep-none ' . $active_class .  $class .' ">';
            }
			...
			if(($this->request->params['controller']=='users')&&($this->request->params['action']=='admin_stats')) {
				...
				$tour_class= 'data-bootstro-step='.$pos.' data-bootstro-content="'.__l($menu['data-bootstro-content']).'" data-bootstro-title="'.__l($menu['title']).'" data-bootstro-placement="bottom"';
			}
			...
			if ($menu['icon-class'] == 'admin-payment') {
				$out.= '<span class="show dc no-mar text-46"><i class="no-mar no-pad over-hide icon-money show"></i></span>';				
			}
			else{
				$out.= '<span class="show dc no-mar text-46 "><i class="no-mar no-pad over-hide show icon-'.$menu['icon-class'].'"></i></span>';
			}
			$out.= '<span class="show">';
            $out.= __l($menu['title']);
            $out.= '</span></a>';
                ...
                $out.= "<h6 class='textb'>";
                $out.= Configure::read('item.alt_name_for_item_singular_caps') . ' ' . __l('Type Builder');
                $out.= "</h6>";
                $out.= "<h6>";
                $out.= $this->Html->link(sprintf(__l('%s Form for Categories'), Configure::read('item.alt_name_for_item_singular_caps')), array(
                    'controller' => 'categories',
                    'action' => 'index'
                ),array('class' => 'graydarkc', 'escape'=>false));
                $out.= "</h6>";
                $out.= '<div class="hor-space sfont textn no-mar"><i class="icon-exclamation-sign hor-smspace"></i>';
                $out.= sprintf(__l('Customize %s form and preview for Categories'), Configure::read('item.alt_name_for_item_singular_small'));
                $out.= "</div>";
                $out.= "</div>";
                $out.= "</div>";
                $out.= "</li>";
                ...
                if (!empty($menu['children'])) {
                    if (!empty($menu['column']) && $menu['column'] > 1) {
						...
						if ($menu['icon-class'] == 'cog') {
							$out.= '<li>';
							$out.= $this->Html->link('<span class="pull-right"><i class="icon-cog"></i>'.__l('Settings Overview').'</span>',array('controller' => 'settings', 'action'=>'index'),array('class' => 'clearfix', 'escape'=>false,'title' => __l('Settings Overview')));
							$out.= '</li>';
						}
						...
						if (empty($menu['is_hide_title'])) {
							$out.= '<li><h6><span class = "span textb">' . __l($menu['title']) . '</span></h6></li>';
						}
						$out.= '<li class="span11 no-mar"><div class="row"><div class="span6"><div class="hor-mspace">';						
                        for ($i = 1; $i <= $menu['column']; $i++) {
                            if ($i == 1) {
								...
								$out.= '</div></div><div class="span5"><div class="hor-mspace">';
                                $from = $to+1;
                                $to = count($menu['children']);
                                $li_class = 'admin-sub-links-right';
                            }
							$out.= '<ul class="unstyled clearfix setting-menu">';
                            $out.= $this->_getLinks($menu, $from, $to);
                            $out.= '</ul>';
                        }
                        $out.= '</div></div></div></li></ul>';
                    } else {
                        $out.= '<ul class="dropdown-menu dl">';
        ...
        foreach($menu['children'] as $child) {
            if ((empty($from) && empty($to)) || ($i >= $from && $i <= $to)) {
				...
				if(!empty($child['url']['controller']) && $child['url']['controller'] == 'payment_gateways') {
					$icon.= '<i class="icon-cog text-16 pull-left no-mar"></i>';
				}
                if (!empty($child['url'])) {
				    $child['htmlAttributes']['class'] = 'graydarkc show left-space';
                    ...
                    $out.= '<h6 class="nav-header"><span class="textb blackc">' . __l($child['title']) . '</span></h6>';
                }
               $out.= '</li>';
            }
            $i++;
