	...
	<?php echo $this->Form->create('PaymentGateway', array('class' => 'form-horizontal space setting-add-form add-live-form'));?>
		<?php
			...
			foreach($paymentGatewaySettings as $paymentGatewaySetting) {
				...
				if($paymentGatewaySetting['PaymentGatewaySetting']['name'] == 'is_enable_for_item_listing_fee'):
					$options['label'] = sprintf(__l('Enable for post %s'), Configure::read('item.alt_name_for_item_singular_small'));
				elseif($paymentGatewaySetting['PaymentGatewaySetting']['name'] == 'is_enable_for_item_verified_fee'):
					$options['label'] = sprintf(__l('Enable for %s verified'), Configure::read('item.alt_name_for_item_singular_small'));
				...
				elseif($paymentGatewaySetting['PaymentGatewaySetting']['name'] == 'is_enable_for_book_a_item'):
					$options['label'] = __l('Enable for book') . ' ' . Configure::read('item.alt_name_for_item_singular_small');
				...
				if ($paymentGatewaySetting['PaymentGatewaySetting']['name'] == 'is_enable_for_signup_fee' || $paymentGatewaySetting['PaymentGatewaySetting']['name'] == 'is_enable_for_item_listing_fee' || $paymentGatewaySetting['PaymentGatewaySetting']['name'] == 'is_enable_for_item_verified_fee' || $paymentGatewaySetting['PaymentGatewaySetting']['name'] == 'is_enable_for_add_to_wallet' || $paymentGatewaySetting['PaymentGatewaySetting']['name'] == 'is_enable_for_book_a_item'):
		...
		<div class="clearfix">
          ...
          <?php
           // $options['value'] = $paymentGatewaySetting['PaymentGatewaySetting']['live_mode_value'];
            //echo $this->Form->input("PaymentGatewaySetting.{$paymentGatewaySetting['PaymentGatewaySetting']['id']}.live_mode_value", array('type' => 'text', 'label' => false, 'div' => false, 'value' => $paymentGatewaySetting['PaymentGatewaySetting']['live_mode_value']));
          ?>
