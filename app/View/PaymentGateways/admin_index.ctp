...
<div class="space js-response">
  ...
  <table class="table table-striped table-bordered table-condensed table-hover no-mar">
    ...
    <tr>
      ...
      <th class="dc"><?php echo __l('Post') . ' ' . Configure::read('item.alt_name_for_item_singular_caps');?></th>
	  <th class="dc"><?php echo __l('Book') . ' ' . Configure::read('item.alt_name_for_item_singular_caps');?></th>
    ...
    <tr>
      ...
      <div class="dropdown top-space">
        <a href="#" title="<?php echo __l('Actions');?>" data-toggle="dropdown" class="icon-cog cur blackc text-20 dropdown-toggle js-no-pjax"><span class="hide"><?php echo __l('Actions');?></span></a>
    ...
    <?php
      unset($items_item_enabled);
      unset($items_verified_enabled);
      unset($items_booking_enabled);
      unset($wallet_enabled);
	  unset($signup_enabled);
      foreach($paymentGateway['PaymentGatewaySetting'] as $paymentGatewaySetting):
      if ($paymentGatewaySetting['name'] == 'is_enable_for_item_listing_fee'):
        $items_item_enabled = $paymentGatewaySetting['test_mode_value'];
      endif;
      if ($paymentGatewaySetting['name'] == 'is_enable_for_book_a_item'):
        $items_booking_enabled = $paymentGatewaySetting['test_mode_value'];
      endif;
    ...
    <?php } ?>
    <td class="dc">
      <?php echo $this->Html->link((!empty($items_item_enabled)) ? '<i class="icon-ok text-16 top-space"></i><span class="hide">Yes</span>' : '<i class="icon-remove text-16 top-space"></i><span class="hide">No</span>', array('action' => 'update_status', $paymentGateway['PaymentGateway']['id'], ConstMoreAction::ItemListing, 'toggle' => (!empty($items_item_enabled)) ? 0 : 1), array('escape' => false, 'class' => 'js-admin-update-status js-no-pjax')); ?>
    </td>
    <td class="dc">
      <?php echo $this->Html->link((!empty($items_booking_enabled)) ? '<i class="icon-ok text-16 top-space"></i><span class="hide">'.__l('Yes') .'</span>' : '<i class="icon-remove text-16 top-space"></i><span class="hide">'.__l('No').'</span>', array('action' => 'update_status', $paymentGateway['PaymentGateway']['id'], ConstMoreAction::ItemBooking, 'toggle' => (!empty($items_booking_enabled)) ? 0 : 1), array('escape' => false, 'class' => 'js-admin-update-status js-no-pjax'));?>
    </td>
