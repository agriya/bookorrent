...
<ul class="breadcrumb top-mspace ver-space">
              ...
              <li class="active"><?php echo __l('User Views');?> </li>
            </ul>
	...
	<tr class=" well no-mar no-pad">
        ...
        if (!empty($userViews)):
            ...
            foreach ($userViews as $userView):
                ...
                ?>
                <tr<?php echo $class;?>>
                    <td class="dc"><?php echo $this->Form->input('UserView.'.$userView['UserView']['id'].'.id', array('type' => 'checkbox', 'id' => "admin_checkbox_".$userView['UserView']['id'], 'label' => "", 'class' => 'js-checkbox-list')); ?></td>
					<td class=""><div class="dropdown"> <span title="<?php echo __l('Actions');?>" data-toggle="dropdown" class="graydarkc left-space hor-smspace icon-cog text-18 cur dropdown-toggle"> <span class="hide"><?php echo __l('Action');?></span> </span>
					...
					<?php if(!empty($userView['Ip']['ip'])): ?>
                            <?php echo  $this->Html->link($userView['Ip']['ip'], array('controller' => 'users', 'action' => 'whois', $userView['Ip']['ip'], 'admin' => false), array('class' => 'js-no-pjax', 'target' => '_blank', 'title' => __l('whois') .' '.$userView['Ip']['host'], 'escape' => false));
							?>
							<div>
							<?php
                            ...
                            ?>
                            <p class="htruncate js-bootstrap-tooltip span2" title="<?php echo $userView['Ip']['City']['name'];?>"><?php echo $this->Html->cText($userView['Ip']['City']['name'], false); ?>   </p>
                            <?php endif; ?></div>
