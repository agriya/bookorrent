...
class TreeHelper extends AppHelper
{
    ...
    public function generate($data, $settings = array()) 
    {
        ...
        foreach($data as $i => $result) {
            ...
            if ($this->__settings['show_add_role']) {
                $return.= ' ';
                $return.= $this->Html->link(__l('Add Role') , array(
                    'controller' => 'roles',
                    'action' => 'add',
                    $result[$model]['id']
                ) , array(
            ...
            if ($this->__settings['show_edit_role']) {
                $return.= $this->Html->link(__l('Edit Role') , array(
                    'controller' => 'roles',
                    'action' => 'edit',
                    $result[$model]['id']
                ) , array(
            ...
            if ($this->__settings['show_delete_role']) {
                $return.= $this->Html->link(__l('Delete Role') , array(
                    'controller' => 'roles',
                    'action' => 'delete',
                    $result[$model]['id']
                ) , array(
                    'class' => 'delete js-confirm',
                    'title' => __l('Delete Role')
                ));
            }
            if ($this->__settings['show_move_role']) {
                $return.= $this->Html->link(__l('Move Role') , '#', array(
                    'class' => 'move',
                    'title' => __l('Move Role')
                ));
            }
     ...
     * to set listings of the holding type - pass 'previous' for the 4th param
     * i.e.
     * // Hide children (2)
