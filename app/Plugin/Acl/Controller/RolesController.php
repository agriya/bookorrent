...
class RolesController extends AppController
{
    ...
    function admin_add($parent_id = null) 
    {
        ...
        if (!empty($this->request->data)) {
            ...
            if ($this->Role->save($this->request->data)) {
                ...
                $this->Session->setFlash(__l('Role has been added') , 'default', null, 'success');
                $this->redirect(array(
                    'action' => 'index'
                ));
            } else {
                $this->Session->setFlash(__l('Role could not be added. Please, try again.'), 'default', null, 'error');
            }
        }
    ...
    {
        $this->pageTitle = __l('Edit Role');
        if (is_null($id)) {
            $this->cakeError('error404');
        }
        if (!empty($this->request->data)) {
            if ($this->Role->save($this->request->data)) {
                $this->Session->setFlash(__l('Role has been updated') , 'default', null, 'success');
            } else {
                $this->Session->setFlash(__l('Role could not be updated. Please, try again.') , 'default', null, 'error');
            }
        } else {
    ...
    function admin_delete($id = null) 
    {
        ...
        if ($this->Role->delete($id)) {
            $this->Session->setFlash(__l('Role deleted'), 'default', null, 'success');
            $this->redirect(array(
                'action' => 'index'
            ));
        } else {
    ...
    {
        $this->pageTitle = __l('Edit Permission');
        $aclLinks = $this->Role->AclLink->find('all', array(
            'order' => array(
                'AclLink.id' => 'DESC'
            ) ,
