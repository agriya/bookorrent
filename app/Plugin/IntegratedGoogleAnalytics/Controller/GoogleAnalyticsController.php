...
class GoogleAnalyticsController extends AppController
{
	...
	public function admin_form_bounce_chart()
    {
		...
		$item_add_form_bounce_arr = array();
		$item_fund_form_bounce_arr = array();
		// item add form bounce query
		$stats = $this->_getStats($condition, 'ga:totalEvents', array('dimensions' => 'ga:eventLabel', 'sort' => 'ga:eventLabel', 'filters' => 'ga:eventCategory=~Item;ga:eventAction=~ItemPosted;ga:eventLabel=~Step '));
		$item_add_steps_arr = array();
		$item_add_events_arr = array();
		if(!empty($stats->rows)){
		foreach($stats->rows as $stat) {
			$item_add_steps_arr[] = "'" . $stat[0] . "'";
			$item_add_events_arr[] = $stat[1];
		}
		}
		$item_add_form_bounce_arr[] = '["Form", ' . implode(', ', $item_add_steps_arr) . ']';
		$item_add_form_bounce_arr[] = '["Item Post", ' . implode(', ', $item_add_events_arr) . ']';			
		$this->set('item_add_form_bounces', implode(',', $item_add_form_bounce_arr));
		// item book form bounce query
		$stats = $this->_getStats($condition, 'ga:totalEvents', array('dimensions' => 'ga:eventLabel', 'sort' => 'ga:eventLabel', 'filters' => 'ga:eventCategory=~ItemUser;ga:eventAction=~Bookit;ga:eventLabel=~Step '));
		$item_fund_events_arr = array();
		$item_fund_steps_arr = array();
		if (!empty($stats->rows)) {
			foreach($stats->rows as $stat) {
				$item_fund_steps_arr[] = "'" . $stat[0] . "'";
				$item_fund_events_arr[] = $stat[1];
			}
			if (!empty($item_fund_events_arr)) {
				$item_fund_form_bounce_arr[] = '["Form", ' . implode(', ', $item_fund_steps_arr) . ']';
				$item_fund_form_bounce_arr[] = '["Item Booking", ' . implode(', ', $item_fund_events_arr) . ']';
			}
		}
		$this->set('item_fund_form_bounces', implode(',', $item_fund_form_bounce_arr));
	}
    ...
    public function admin_oauth_login()
    {
		if (!empty($_GET['code'])) {
			...
			$this->Session->setFlash(__l('Google Analytics credentials has been updated'), 'default', null, 'success');
			$this->redirect(array(
				'controller' => 'settings',
				'action' => 'edit',
				38
