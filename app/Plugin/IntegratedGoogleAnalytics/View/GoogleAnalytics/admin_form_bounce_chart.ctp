<div class="js-form-bounces js-cache-load-admin-chart-bounces-form">
  <div class="accordion-group">
   <div class="accordion-heading bootstro" data-bootstro-step="1" data-bootstro-content="<?php echo sprintf(__l("Bounces represents the percentage of visitors who enter the site and 'bounce' (leave the site) rather than continue viewing other pages within the site. Bounce rates can be used to help determine the effectiveness or performance of an entry page. An entry page with a low bounce rate means that the page effectively causes visitors to view more pages and continue on deeper into the web site. A pageview is an instance of a page being loaded by a browser. The Pageviews metric is the total number of pages viewed; repeated views of a single page are also counted. Visitors is the number of user visit your site. Bounces represents the percentage of visitors who enter the site and 'bounce' (leave the site) rather than continue viewing other pages within the site. The graph is use to trace step wise completion of user's %s post/book so it helps admin to find in which step user interrupt without complete their posting/booking. This analysis helps admin to improve the clarity of %s post steps thereby increase the %s post/book."), Configure::read('item.alt_name_for_item_singular_small'), Configure::read('item.alt_name_for_item_singular_small'), Configure::read('item.alt_name_for_item_singular_small'));?>" data-bootstro-placement='bottom' data-bootstro-width="600px" data-bootstro-title="Bounces">
      <div class="well no-pad no-mar no-bor clearfix box-head">
        <h5> <span class="space pull-left"> <i class="icon-bar-chart no-bg"></i> <?php echo __l('Bounces'); ?> </span> <a class="accordion-toggle js-toggle-icon js-no-pjax grayc no-under clearfix pull-right" href="#form-bounces" data-parent="#accordion-admin-dashboard" data-toggle="collapse"> <i class="icon-angle-up pull-right text-16 textb"></i> </a>
          <div class="dropdown pull-right ver-space"> <a class="dropdown-toggle js-no-pjax js-overview grayc no-under" data-toggle="dropdown" href="#"> <i class="icon-wrench no-pad"></i> </a>
        ...
        <div class="row-fluid ver-space">
          ...
          <div class="span24">
			...
			?>
				...
				<h3><?php echo Configure::read('item.alt_name_for_item_singular_caps') . ' ' . __l('Post Form Bounces'); ?></h3>
				  <script type="text/javascript">
					  ...
					  <?php } ?>
						var data = google.visualization.arrayToDataTable([
						  <?php echo ${'item_add_form_bounces'}; ?>
						]);
						var chart = new google.visualization.BarChart(document.getElementById('form_bounces_chart_item_post'));
						chart.draw(data);
					  <?php if(empty($this->request->params['isAjax'])) { ?>
					  }
					  <?php } ?>
					</script>
				  <div class="dc" id="form_bounces_chart_item_post" style="width: 450px; height: 400px;"></div>
				</section>
            <section class="span12 no-mar dc">
			  <h3><?php echo Configure::read('item.alt_name_for_item_singular_caps') . ' ' . __l('Book Form Bounces'); ?></h3>
              <script type="text/javascript">
                  <?php if(empty($this->request->params['isAjax'])) { ?>
				  google.setOnLoadCallback(drawChart);
                  function drawChart() {
				  <?php } ?>
                    var data = google.visualization.arrayToDataTable([
					  <?php echo ${'item_fund_form_bounces'}; ?>
                    ]);
                    var chart = new google.visualization.BarChart(document.getElementById('form_bounces_chart_item_fund'));
                    chart.draw(data);
                  <?php if(empty($this->request->params['isAjax'])) { ?>
                  }
				  <?php } ?>
                </script>
              <div class="dc" id="form_bounces_chart_item_fund" style="width: 450px; height: 400px;"></div>
            </section>
          </div>
        </div>
      </div>
