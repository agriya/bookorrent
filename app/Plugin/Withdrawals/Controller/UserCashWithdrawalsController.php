...
class UserCashWithdrawalsController extends AppController
{
    ...
    public function index() 
    {
        ...
        if ($this->RequestHandler->prefers('json')) {
            $response = Cms::dispatchEvent('Controller.User.Withdrawals', $this, array());
        }
    }
    public function add() 
    {
        ...
        if (!empty($this->request->data)) {
			if ($this->RequestHandler->prefers('json')) {
				if(!empty($this->request->data)){
					$this->request->data['UserCashWithdrawal'] = $this->request->data;
				}
			}
            ...
            if (($this->request->data['UserCashWithdrawal']['user_id'] != $this->Auth->user('id')) || ($this->request->data['UserCashWithdrawal']['role_id'] != $this->Auth->user('role_id'))) {
				...
				if ($this->RequestHandler->prefers('json')) {
					$message = array("message" => __l('Invalid request'), "error" => 0);
				}else{
            ...
            if ($this->UserCashWithdrawal->validates() &$this->UserCashWithdrawal->validationErrors['amount'] = $this->UserCashWithdrawal->_checkAmount($this->request->data['UserCashWithdrawal']['amount'])) {
                ...
                if ($this->UserCashWithdrawal->save($this->request->data)) {
                    ...
                    $this->Session->setFlash(__l('Withdraw Fund Request has been added'), 'default', null, 'success');
					$ajax_url = Router::url(array(
							'controller' => 'user_cash_withdrawals',
							'action' => 'index'
					));
                    if ($this->RequestHandler->prefers('json')) {
						$message = array("message" => __l('Withdraw Fund Request has been added'), "error" => 0);
					}else{
                    ...
                    $this->Session->setFlash(__l('Withdraw Fund Request could not be added. Please, try again.'), 'default', null, 'error');
					$message = array("message" => __l('Withdraw Fund Request could not be added. Please, try again.'), "error" => 1);
                }
            } else {
                $this->Session->setFlash(__l('Withdraw Fund Request could not be added. Please, try again.'), 'default', null, 'error');
				$message = array("message" => __l('Withdraw Fund Request could not be added. Please, try again.'), "error" => 1);
		...
		if ($this->RequestHandler->prefers('json')) {
			Cms::dispatchEvent('Controller.User.WithdrawalsAdd', $this, array(
				'data' => $message
			));
		}	
    }
    public function admin_index() 
    {
        ...
        $this->pageTitle = __l('Withdraw Fund Requests').' - '.__l('from Users');
