	   ...
	   $user = $this->AffiliateRequest->User->find('first', array(
        ...
        if (!empty($user['User']['is_affiliate_user'])) {
			if ($this->RequestHandler->prefers('json')) {
				Cms::dispatchEvent('Controller.Affiliate.AffiliateRequest', $this, array(
				'data' => array(
							'error' => 0,
							'message' =>__l('You have already affiliate user')
						)
				));				
			}else{
				$this->redirect(array(
        ...
        if (!empty($this->request->data)) {
			...
			if ($this->RequestHandler->prefers('json')) {
				if(!empty($this->request->data)){
					$this->request->data['AffiliateRequest'] = $this->request->data;
				}
			}		
            ...
            if ($this->AffiliateRequest->save($this->request->data)) {
				...
				if ($this->RequestHandler->prefers('json')) {
						$message =  array(
							'error' => 0,
							'message' =>__l('Your request added successfully')
						);
				}else{
				...
				if ($this->RequestHandler->prefers('json')) {
						$message =  array(
							'error' => 1,
							'message' =>__l('Affiliate request could not be added. Please, try again.')
						);
				}else{
