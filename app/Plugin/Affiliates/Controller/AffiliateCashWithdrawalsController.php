...
class AffiliateCashWithdrawalsController extends AppController
{
    ...
    {
        $this->pageTitle = __l('Affiliate Cash Withdrawal Requests');
        $conditions = array();
        ...
        if ($this->RequestHandler->prefers('json')) {			
            $response = Cms::dispatchEvent('Controller.Affiliate.CashWithdrawal', $this, array());
        }			
        $this->set('user', $user);
    ...
    public function add() 
    {
        ...
        if (!empty($this->request->data)) {
			// todo: json api call changes need to fix
			if ($this->RequestHandler->prefers('json')) {
				if(!empty($this->request->data)){
					$this->request->data['AffiliateCashWithdrawal'] = $this->request->data;
				}
			}
            ...
            if ($this->AffiliateCashWithdrawal->validates() &$this->AffiliateCashWithdrawal->validationErrors['amount'] = $this->AffiliateCashWithdrawal->_checkAmount($this->request->data['AffiliateCashWithdrawal']['amount'])) {
                ...
                if ($this->AffiliateCashWithdrawal->save($this->request->data)) {
                    ...
                    $this->Session->setFlash(__l('Affiliate cash withdrawal request has been added') , 'default', null, 'success');
                    $ajax_url = Router::url(array(
                        'controller' => 'affiliate_cash_withdrawals',
                        'action' => 'index'
                    ));
					if ($this->RequestHandler->prefers('json')) {
						$message = array("message" => __l('Affiliate cash withdrawal request has been added'), "error" => 0);
					}else{
                    ...
                    $this->Session->setFlash(__l('Affiliate cash withdrawal request could not be added. Please, try again.') , 'default', null, 'error');
					$message = array("message" => __l('Affiliate cash withdrawal request could not be added. Please, try again.'), "error" => 1);
                }
            } else {
                $this->Session->setFlash(__l('Affiliate cash withdrawal request could not be added. Please, try again.') , 'default', null, 'error');
				$message = array("message" => __l('Affiliate cash withdrawal request could not be added. Please, try again.'), "error" => 1);
		...
		if ($this->RequestHandler->prefers('json')) {
			Cms::dispatchEvent('Controller.Affiliate.CashWithdrawalAdd', $this, array(
			'data' => $message
			));		
		}
    }
