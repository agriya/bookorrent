 ...
 <table class="table list no-round">
	...
	<tr class=" well no-mar no-pad js-even">
      ...
      <td class="dl">
        ...
        <?php echo  $this->Html->link($subscription['Ip']['ip'], array('controller' => 'subscriptions', 'action' => 'whois', $subscription['Ip']['ip'], 'admin' => false), array('class' => 'js-no-pjax', 'target' => '_blank', 'class' => 'js-no-pjax', 'title' => 'whois '.$subscription['Ip']['ip'], 'escape' => false));
        ?>
        <p>
        <?php
      ...
      ?>
    <tr>
      <td colspan="5" class="errorc space"><i class="icon-warning-sign errorc"></i> <?php echo __l('No Users available');?></td>
    </tr>
      <?php
      endif;
      ?>
