...
for($i=1;$i<=$page;$i++) {
  ...
  if (!empty($Subscriptions)) {
	...
	if($this->request->params['named']['filter_id'] ==  ConstMoreAction::PrelaunchSubscribed){
	  ...
	  __l('Invitation Sent') => ($Subscription['Subscription']['is_sent_private_beta_mail']==1)?__l("Yes"):__l("No"),
	  ...
	  $data[]['Subscription'] = array(
      __l('Email') => $Subscription['Subscription']['email'], 
	  __l('Registered') => ($Subscription['Subscription']['is_sent_private_beta_mail']==1)?__l("Yes"):__l("No"),
	  __l('Registration From Friends Invite') => ($Subscription['Subscription']['is_invite']==1)?__l("Yes"):__l("No"),
