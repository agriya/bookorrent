...
?>
<section id="main" class="social_marketings-publish clearfix bot-space container xltriggered">
<section class="<?php echo $class; ?>">
  <div>
    <div class="clearfix"><h3 class="ver-space text-32"><?php echo $this->Html->cText($this->pageTitle, false); ?></h3></div>
    ...
    <div class="clearfix">
      <div class="<?php echo ($this->request->params['named']['type'] != 'others') ? 'span12' : ''; ?>">
        ...
        <?php if ($this->request->params['named']['type'] == 'facebook') { ?>
          ...
          ?>
          <div class="loader" id="js-loader" data-fb_connect="<?php echo $fb_connect; ?>" data-fb_app_id="<?php echo Configure::read('facebook.app_id'); ?>" data-item_url="<?php echo $request_url; ?>"  data-request_name="<?php echo urlencode($this->Html->cText($request['Request']['title'], false)); ?>" data-caption="<?php echo urlencode($this->Html->cText($request['Request']['information'], false)); ?>" data-description="<?php echo urlencode($this->Html->cText($request['Request']['description'], false)); ?>" data-redirect_url="<?php echo $redirect_url; ?>" data-type="iframe">
            <!-- data-type="popup" -> set that popup to load share other than loaded to iframe -->
            <span id="js-FB-Share-description" class="hide"><?php echo $this->Html->cText($request['Request']['description'], false); ?></span>
            <span id="js-FB-Share-title"  class="hide"><?php echo $this->Html->cText($request['Request']['title'], false); ?></span>
			<span id="js-FB-Share-caption"  class="hide"><?php echo $this->Html->cText($request['Request']['information'], false); ?></span>
        ...
        <?php
			...
			'##ITEM_NAME##' => $this->Html->cText($request['Request']['information'], false),
		...
		<div id="js-twitter" class="hor-space mspace">
			<a href="https://twitter.com/share" class="twitter-share-button" data-url="<?php echo $this->Html->cText($request_url, false); ?>" data-text="<?php echo $default_content;?>" data-count="none" data-size="large">Tweet</a>
        ...
        <?php } else if($this->request->params['named']['type'] == 'others') { ?>
          <div class="clearfix space mspace" id="js-others">
            <ul class="unstyled span19 other-social nav-pills clearfix top-space">
              <li class="span6"><div><a href="https://www.linkedin.com/cws/share?url=<?php echo $this->Html->cText($request_url, false);?>" class="twitter-share-button no-under" target="_blank"><i class="icon-linkedin-sign text-32 pull-left linkedc"></i><span class="text-14 <?php echo $span_class;?> blackc"><?php echo __l('Share about this request on LinkedIn');?></span></a>
              </div></li>
              <li class="span6"><div><a href="https://plus.google.com/share?url=<?php echo $this->Html->cText($request_url, false);?>" onclick="javascript:window.open(this.href,  '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');return false;" class="no-under"><i class="pull-left icon-google-plus-sign text-32 googlec"></i><span class="text-14  <?php echo $span_class;?>  blackc"><?php echo __l('Share about this request on Google+'); ?></span></a></div></li>
              <li class="span6"><div><a href="http://pinterest.com/pin/create/button/?url=<?php echo $this->Html->cText($request_url, false);?>&description=<?php echo $this->Html->cText($request['Request']['information'], false); ?>" target="_blank" class="no-under"><i class="pull-left icon-pinterest-sign text-32 pinterestc"></i><span class="text-14 <?php echo $span_class;?>  blackc"><?php echo __l('Share about this request on Pinterest');?></span></a></div></li>
            </ul>
          </div>
        <?php } ?>
      </div>
    ...
    <div class="clearfix form-actions dr span19 js-skip-show hide">
      <?php
        if ($this->request->params['named']['type'] == 'others') {
          echo $this->Html->link('Done', array('controller' => 'requests', 'action' => 'view', $request['Request']['slug']), array('title' => 'Done','class' => 'pull-right js-bootstrap-tooltip'));
        } else {
          echo $this->Html->link(__l('Skip') . ' >>', array('controller' => 'social_marketings', 'action' => 'publish_request', !empty($id) ? $id : '', 'type' => $next_action, 'publish_action' => $this->request->params['named']['publish_action']), array('title' => __l('Skip'), 'class' => 'pull-right blackc js-bootstrap-tooltip ' . $js_skip_btn));
        }
      ?>
    </div>
	</div>
