...
class UserFollowersController extends AppController
{
    ...
    public function index($user_id = null)
    {
        ...
        if ($this->RequestHandler->prefers('json')) {
            $response = Cms::dispatchEvent('Controller.User.UserFollower', $this, array());
        }
    }
    ...
    public function add($username = null)
    {
        ...
        if (!empty($user)) {
            if ($user['User']['id'] != $this->Auth->user('id')) {
                    ...
                    if ($this->UserFollower->save($this->request->data)) {
						...
						if ($this->RequestHandler->prefers('json')) {
							$message = array(
										...
										'unfollow' => Router::url(array('controller' => 'user_followers','action' => 'delete',$follower_id), true)
									);
						}else{
						...
						if ($this->RequestHandler->prefers('json')) {
							$message = array(
										'error' => 1,
										'message' => __l('User could not be added as follower. Please, try again')
									);
						}else{
				...
				if ($this->RequestHandler->prefers('json')) {
					$message = array(
								'error' => 1,
								'message' => __l('You can not follow yourself')
							);
				}else{
		...
		// <-- For iPhone App code
        if ($this->RequestHandler->prefers('json')) {
            $response = Cms::dispatchEvent('Controller.User.UserFollowerAdd', $this, array(
				'message' => $message
			));
        }
    }
    public function delete($id = null)
    {
        ...
        if ($this->UserFollower->delete($id)) {
			...
			if ($this->RequestHandler->prefers('json')) {
				$message = array(
							...
							"follow" => Router::url(array('controller' => 'user_followers','action' => 'add', $userFollower['FollowUser']['username'], 'ext' => 'json'), true)
						);
			}else{
				if ($this->RequestHandler->isAjax()) {
					$ajax_url = Router::url(array(
		...
		// <-- For iPhone App code
        if ($this->RequestHandler->prefers('json')) {
            $response = Cms::dispatchEvent('Controller.User.UserFollowerDelete', $this, array(
				'message' => $message
			));
        }		
    }
    public function unfollow($id = null)
    {
        ...
        if ($this->RequestHandler->prefers('json') && !empty($_GET['key'])) {
            Cms::dispatchEvent('Controller.Item.unfollow', $this, array(
                'data' => $response
            ));
        }
    }
