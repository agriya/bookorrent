    ...
    public function add($request_id = null)
    {
        if (!empty($this->request->data)) {
			...
			if ($this->RequestHandler->prefers('json')) {
				$this->request->data['Request']['id'] = $this->request->data['request_id'];
				unset($this->request->data['request_id']);
				$this->request->data['RequestFlag'] = $this->request->data;
			}
            ...
            if ($this->RequestFlag->save($this->request->data)) {
				...
				$this->set('iphone_response', array("message" => __l('Flag has been added'), "error" => 0));
				...
				$this->set('iphone_response', array("message" =>__l('Flag could not be added. Please, try again.'), "error" => 1));
		...
		// <--- Iphone json code
		if ($this->RequestHandler->prefers('json')) {
			$response = Cms::dispatchEvent('Controller.RequestFlag.Add', $this, array());
		}		
    }
