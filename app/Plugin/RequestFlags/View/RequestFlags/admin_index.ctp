...
<ul class="breadcrumb top-mspace ver-space">
              ...
              <li class="active"><?php echo __l('Request Flags'); ?> </li>
            </ul> 
<?php endif; ?>			
...
?>
	<tr<?php echo $class;?>>
	<?php if(empty($this->request->params['named']['view_type'])) : ?>
		<td class="dc"><?php echo $this->Form->input('RequestFlag.'.$requestFlag['RequestFlag']['id'].'.id', array('type' => 'checkbox', 'id' => "admin_checkbox_".$requestFlag['RequestFlag']['id'], 'label' => "", 'class' => 'js-checkbox-list')); ?></td>
		<?php endif; ?>
		 <td class="dc">
			<span class="dropdown"> <span title="<?php echo __l('Actions');?>" data-toggle="dropdown" class="graydarkc left-space hor-smspace icon-cog text-18 cur dropdown-toggle"> <span class="hide"><?php echo __l('Actions'); ?></span> </span>
		...
		<td class="dl"><div class="htruncate js-bootstrap-tooltip span7" title="<?php echo $this->Html->cText($requestFlag['Request']['title'], false); ?>"><?php echo $this->Html->link($this->Html->cText($requestFlag['Request']['title']), array('controller'=> 'requests', 'action'=>'view', $requestFlag['Request']['slug'], 'admin' => false), array('class'=>'js-no-pjax','escape' => false));?></div></td>
		<td class="dl"><?php echo $this->Html->cText($requestFlag['RequestFlagCategory']['name']);?></td>
		<td class="dl"><div class="htruncate js-bootstrap-tooltip span7" title="<?php echo $this->Html->cHtml($requestFlag['RequestFlag']['message'], false); ?>"><?php echo $this->Html->cText($requestFlag['RequestFlag']['message']);?></div></td>
		<td class="dc">
			    <?php
                 if(!empty($requestFlag['Ip']['ip'])): ?>
                            ...
                            if(!empty($requestFlag['Ip']['Country'])):
                                ?>
                                <span class="flags flag-<?php echo $this->Html->cText(strtolower($requestFlag['Ip']['Country']['iso_alpha2']), false); ?>" title ="<?php echo $this->Html->cText($requestFlag['Ip']['Country']['name'], false); ?>">
									<?php echo $this->Html->cText($requestFlag['Ip']['Country']['name'], false); ?>
								</span>
                                <?php
                            endif;
							 if(!empty($requestFlag['Ip']['City'])):
                            ?>
                            <p class="htruncate js-bootstrap-tooltip span2" title="<?php echo $this->Html->cText($requestFlag['Ip']['City']['name'], false);?>"><?php echo $this->Html->cText($requestFlag['Ip']['City']['name'], false); ?>    </p>
                            <?php endif; ?></div>                            
                        <?php else: ?>
							<?php echo __l('N/A'); ?>
						<?php endif; ?>
