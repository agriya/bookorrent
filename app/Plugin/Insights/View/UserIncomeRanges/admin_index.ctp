...
<ul class="breadcrumb top-mspace ver-space">
              ...
              <li class="active"><?php echo __l('Income Ranges');?></li>
            </ul> 
...
<?php
	echo $this->Form->create('UserIncomeRange' , array('class' => 'normal','action' => 'update'));
...
<div class="ver-space">
	...
	<tr class=" well no-mar no-pad">
        ...
        <th class="sep-right dl"><?php echo $this->Paginator->sort( 'income',__l('Income'));?></th>
       </tr></thead><tbody>
<?php
...
foreach ($userIncomeRanges as $userIncomeRange):
	...
	if($userIncomeRange['UserIncomeRange']['is_active']):
...
?>
	<tr class="<?php echo $class.' '.$active_class;?>">
		<td class="dc"><?php echo $this->Form->input('UserIncomeRange.'.$userIncomeRange['UserIncomeRange']['id'].'.id', array('type' => 'checkbox', 'id' => "admin_checkbox_".$userIncomeRange['UserIncomeRange']['id'], 'label' => "", 'class' => $status_class.' js-checkbox-list')); ?></td>
		<td class="dc"><span class="dropdown"> <span title="<?php echo __l('Actions');?>" data-toggle="dropdown" class="graydarkc left-space hor-smspace icon-cog text-18 cur dropdown-toggle"> <span class="hide"><?php echo __l('Actions');?></span> </span>
					...
					<li><?php echo $this->Html->link('<i class="icon-edit"></i>'.__l('Edit'), array('action' => 'edit', $userIncomeRange['UserIncomeRange']['id']), array('escape' => false,'class' => 'delete', 'title' => __l('Edit')));?></li>
        			<li><?php echo $this->Html->link('<i class="icon-remove"></i>'.__l('Delete'), array('action' => 'delete', $userIncomeRange['UserIncomeRange']['id']), array('escape' => false,'class' => 'delete js-delete', 'title' => __l('Delete')));?></li>
			   </ul>
							 </span>
                            </td>
		<td class="dc"><?php echo $this->Html->cDateTime($userIncomeRange['UserIncomeRange']['created']);?></td>
		<td class="dl"><?php echo $this->Html->cText($userIncomeRange['UserIncomeRange']['income']);?></td>
		</tr>
<?php
    endforeach;
else:
?>
	<tr>
		<td colspan="6" >
			<div class="space dc">
				<p class="ver-mspace grayc top-space text-16 "><?php echo __l('No Income Ranges available');?></p>
			</div>
		</td>
	</tr>
<?php
