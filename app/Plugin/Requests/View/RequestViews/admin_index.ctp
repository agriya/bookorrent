...
<ul class="breadcrumb top-mspace ver-space">
              ...
              <li class="active"><?php echo __l('Request Views'); ?>   </li>
            </ul> 
            <div class="tabbable ver-space sep-top top-mspace">
                <div id="list" class="tab-pane active in no-mar">
<div class="clearfix dc">
					<?php echo $this->Form->create('RequestView', array('method' => 'type', 'class' => 'form-search bot-mspace big-input span', 'action'=>'index')); ?>
	...
	<tr class=" well no-mar no-pad">
		...
		<?php 
            ...
            foreach ($requestViews as $requestView):
                ...
                <tr<?php echo $class;?>>
                    <?php if(empty($this->request->params['named']['view_type'])) : ?>
                    <td class="dc"><?php echo $this->Form->input('RequestView.'.$requestView['RequestView']['id'].'.id', array('type' => 'checkbox', 'id' => "admin_checkbox_".$requestView['RequestView']['id'], 'label' => "" , 'class' => 'js-checkbox-list')); ?></td>
					<?php endif; ?>
		 <td class="dc"><span class="dropdown"> <span title="<?php echo __l('Actions');?>" data-toggle="dropdown" class="graydarkc left-space hor-smspace icon-cog text-18 cur dropdown-toggle"> <span class="hide"><?php echo __l('Actions');?></span> </span>
					...
					<?php if(empty($this->request->params['named']['view_type'])) : ?>
	                    <td class="dl"><div class="htruncate js-bootstrap-tooltip span5" title="<?php echo $this->Html->cText($requestView['Request']['title'], false); ?>"><?php echo $this->Html->link($this->Html->cText($requestView['Request']['title']), array('controller'=> 'requests', 'action'=>'view', $requestView['Request']['slug'], 'admin' => false), array('class'=>'js-no-pjax','escape' => false,'title' => $this->Html->cText($requestView['Request']['title'],false)));?></div>
                        </td>
					<?php endif; ?>
                   ...
                   if(!empty($requestView['Ip']['ip'])): ?>
                            <?php echo  $this->Html->link($requestView['Ip']['ip'], array('controller' => 'users', 'action' => 'whois', $requestView['Ip']['ip'], 'admin' => false), array('class'=>'js-no-pjax','target' => '_blank', 'title' => __l('whois').' '.$requestView['Ip']['host'], 'escape' => false));
							?>							
							<?php
                            if(!empty($requestView['Ip']['Country'])):
                                ?>
                                <span class="flags flag-<?php echo $this->Html->cText(strtolower($requestView['Ip']['Country']['iso_alpha2']), false); ?>" title ="<?php echo $this->Html->cText($requestView['Ip']['Country']['name'], false); ?>">
									<?php echo $this->Html->cText($requestView['Ip']['Country']['name'], false); ?>
								</span>
                                <?php
                            endif;
							 if(!empty($requestView['Ip']['City'])):
                            ?>
                            <span class="htruncate js-bootstrap-tooltip span2" title="<?php echo $this->Html->cText($requestView['Ip']['City']['name'], false);?>"><span> 	<?php echo $this->Html->cText($requestView['Ip']['City']['name'], false); ?>    </span></span>
                            <?php endif; ?>
