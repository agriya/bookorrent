...
<ul class="breadcrumb top-mspace ver-space">
            ...
            <div class="tabbable ver-space sep-top top-mspace">
				...
				echo $this->Html->link( '
					<dl class="dc list users '.$class .' mob-clr mob-sep-none ">					         	
						<dt class="pr hor-mspace text-11 grayc"  title="'.__l('User Flaged').'">'.__l('User Flagged').'</dt>
...
<table class="table list no-round" id="js-expand-table">
	...
	<tr class=" well no-mar no-pad js-even">
		...
		<th class="dl graydarkc sep-right"><?php echo $this->Paginator->sort( 'User.username',__l('Username'));?></th>
	   	<th class="dc graydarkc sep-right"><?php echo $this->Paginator->sort( 'item_count',__l('Offered')); ?></th>
...
?><tbody>
	 <tr class="<?php echo $class.' '.$active_class;?> expand-row js-odd">
		<td class="<?php echo $class;?> select dc"><?php echo $this->Form->input('Request.'.$request['Request']['id'].'.id', array('type' => 'checkbox', 'id' => "admin_checkbox_".$request['Request']['id'], 'label' => "",'before' => '<span class="show pull-left hor-smspace"><i class="icon-caret-down"></i></span>', 'class' => $status_class.' js-checkbox-list')); ?></td>
		...
		<td class="dc"><?php echo $this->Html->cInt($request['Request']['item_count']);?></td>
	...
	<tr class="hide">
		...
		<li>	<?php echo $this->Html->link((($request['Request']['is_approved']) ? '<i class="icon-thumbs-down"></i>'.__l('Disapprove') : '<i class="icon-thumbs-up"></i>'.__l('Approve')), array('action' => 'admin_update_status',  $request['Request']['id'], 'status' => (($request['Request']['is_approved']) ? 'disapproved' : 'approved')), array('escape' => false,'class' => 'js-confirm ' . $style_class, 'title' => (( $request['Request']['is_approved']) ? __l('Disapprove') : __l('Approve')))); ?>
			...
			<ul id="myTab3" class="nav nav-tabs top-smspace">
                 <li class="active"><a href="#As-Price-<?php echo $this->Html->cInt($request['Request']['id'], false); ?>" data-toggle="tab"><?php echo __l('Overview');?></a> </li>                                
             </ul>
		<div class="sep-right sep-left sep-bot tab-round tab-content" id="myTabContent3">
		<div class="tab-pane space active" id="As-Price-<?php echo $this->Html->cInt($request['Request']['id'], false); ?>">
					...
					<div class="span13 right-space">
						...
						<div class="clearfix ver-space bot-mspace">
							<dl class="list sep-right list">
								...
								<dd title="" class="textb text-16 graydarkc pr hor-mspace"><?php echo Configure::read('site.currency').$this->Html->cCurrency($request['Request']['price']);?></dd>
							</dl>
							<dl class="list sep-right list">
								<dt class="pr hor-mspace text-11"><?php echo __l('From'); ?></dt>
								<dd title="" class="textb text-16 graydarkc pr hor-mspace"><?php echo $this->Html->cDate($request['Request']['from']);?></dd>
							</dl>
							<dl class="list list">
								<dt class="pr hor-mspace text-11"><?php echo __l('To'); ?></dt>
								<dd title="" class="textb text-16 graydarkc pr hor-mspace"><?php echo $this->Html->cDate(getToDate($request['Request']['to']));?></dd>
							</dl>
						</div>
					</div>
				</div>
					...
					<div class="span13 right-space">
						...
						<div class="clearfix ver-space bot-mspace">
							...
							<dl class="list sep-right">
								...
								<dd title="" class="textb text-16 graydarkc pr hor-mspace"><?php echo $this->Html->cInt($request['Request']['item_count']);?></dd>
							</dl>
							<dl class="list sep-right">
    ...
    if (!empty($requests)) :
        ...
        <div class="admin-select-block ver-mspace pull-left mob-clr dc">
			<div class="span top-mspace">
				...
				<?php echo $this->Html->link(__l('Disapproved'), '#', array('class' => 'js-select js-no-pjax {"checked":"js-checkbox-inactive","unchecked":"js-checkbox-active"} hor-smspace grayc','title' => __l('Disapprove'))); ?>
				<?php echo $this->Html->link(__l('Approved'), '#', array('class' => 'hor-smspace grayc js-select js-no-pjax {"checked":"js-checkbox-active","unchecked":"js-checkbox-inactive"}','title' => __l('Approve'))); ?>
