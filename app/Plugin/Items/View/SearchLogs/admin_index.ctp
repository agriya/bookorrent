...
<ul class="breadcrumb top-mspace ver-space">
              ...
              <li class="active"><?php echo __l('Search Logs'); ?></li>
            </ul> 
...
?>
	<tr<?php echo $class;?>>
    <td class="dc"><?php echo $this->Form->input('SearchLog.'.$searchLog['SearchLog']['id'].'.id', array('type' => 'checkbox', 'id' => "admin_checkbox_".$searchLog['SearchLog']['id'], 'label' => "", 'class' => $status_class.' js-checkbox-list')); ?></td>
		<td class="dc"><span class="dropdown"> <span title="<?php echo __l('Actions');?>" data-toggle="dropdown" class="graydarkc left-space hor-smspace icon-cog text-18 cur dropdown-toggle"> <span class="hide"><?php echo __l('Actions');?></span> </span>
					...
					<li><?php echo $this->Html->link('<i class="icon-search"></i>'.__l('Search'), array('controller'=> 'items', 'action' => 'index',$hash,$salt, 'admin' => false), array('class' => 'search','escape' => false));?></li>
		...
		<td><div class="htruncate js-bootstrap-tooltip span5" title="<?php echo $this->Html->cText($searchLog['SearchKeyword']['keyword'], false);?>"><?php echo $this->Html->cText($searchLog['SearchKeyword']['keyword']);?></div></td>
		...
		<td class="dl">
          	<?php if(!empty($searchLog['Ip']['ip'])): ?>
                            <?php echo  $this->Html->link($searchLog['Ip']['ip'], array('controller' => 'users', 'action' => 'whois', $searchLog['Ip']['ip'], 'admin' => false), array('class' => 'js-no-pjax', 'target' => '_blank', 'title' => __l('whois').' '.$searchLog['Ip']['host'], 'escape' => false));
							?>
							<div>
							<?php
                            if(!empty($searchLog['Ip']['Country'])):
                                ?>
                                <span class="flags flag-<?php echo $this->Html->cText(strtolower($searchLog['Ip']['Country']['iso_alpha2']), false); ?>" title ="<?php echo $this->Html->cText($searchLog['Ip']['Country']['name'], false); ?>">
									<?php echo $this->Html->cText($searchLog['Ip']['Country']['name'], false); ?>
								</span>
                                <?php
                            endif;
							 if(!empty($searchLog['Ip']['City'])):
                            ?>
                            <p class="htruncate js-bootstrap-tooltip span2" title="<?php echo $this->Html->cText($searchLog['Ip']['City']['name'], false);?>"><span> 	<?php echo $this->Html->cText($searchLog['Ip']['City']['name'], false); ?>    </p>
                            <?php endif; ?></div>                           
                        <?php else: ?>
							<?php echo __l('N/A'); ?>
						<?php endif; ?>
