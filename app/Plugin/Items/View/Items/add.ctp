...
<div class ="ver-space clearfix sep-bot top-mspace ">
		<?php if (!empty($is_payout_error) || !empty($this->request->params['named']['step'])) { ?>
				<h2 class="ver-space text-32">
					<div class="span9 "><span class="badge <?php echo empty($this->request->params['named']['step'])?'badge-module':'';?> space span">1</span>&nbsp;<?php echo __l('Payout');?></div>
					<div class="span8 "><span class="badge <?php echo !empty($this->request->params['named']['step'])?'badge-module':'';?> space span">2</span>&nbsp;<?php echo __l('Post a').' '.Configure::read('item.alt_name_for_item_singular_caps');?></div>
				</h2>
	<?php }else{ ?>
	<h2 class="ver-space text-32"><?php echo __l('Post a'). ' ' . Configure::read('item.alt_name_for_item_singular_caps'); ?> </h2>
	<?php } ?>
</div>
	<?php if (!empty($is_payout_error)) { ?>
			<?php if (isset($this->request->query['error_code'])) { ?>
				<?php if ($this->request->query['error_code'] != 0) { ?>
					  <div class="alert alert-error  ver-mspace">
						<?php echo $this->request->query['error_message'] ; ?>
					 </div>
				<?php } else { ?>
					 <div class="alert alert-success  ver-mspace">
						<?php echo "Successfully connected"; ?>
					 </div>
				<?php } ?>
			<?php }
				echo $this->element('sudopay_user_accounts', array('step' => 1 , 'user' => $userDetails['User'], 'redirect_url' => $this->request->url, 'cache' => array('config' => 'sec')), array('plugin' => 'Sudopay'));
	 } else { ?>     
	 <?php
	if (!empty($this->request->data['Item']['request_id']) && (isset($this->request->params['pass'][0]) && $this->request->params['pass'][0]=='request')) {
		echo $this->element('item-index', array('config' => 'sec', 'type' => 'item', 'request_id' => $this->request->data['Item']['request_id'], 'request_latitude' => $this->request->data['Item']['request_latitude'], 'request_longitude' => $this->request->data['Item']['request_longitude']));
	}
	?>
	...
	<?php 
		...
		echo $this->Form->input('session_key', array('type' => 'hidden', 'value' => md5(rand(100000, 1000000))));
	...
	?>
		<div class='clearfix'>
			<?php
			if (!empty($this->request->data['Item']['request_id'])) {
		...
		<div class="clearfix top-space top-mspace js-formfields-responses">
			<?php if (!empty($this->request->data['Item']['sub_category_id'])) { ?>
					...
					?>
						<div class="clearfix">
								<h3 class="well space textb text-16"><?php echo $this->Html->cText($FormFieldGroup['FormFieldGroup']['name'], false); ?></h3>
								<?php 
		...
		<div class="form-actions">
			<div class="fileupload-buttonbar submit pull-right">
				<button class="btn btn-primary btn-large start js-upload-form-submit" type="button"><span><?php echo __l('Add'); ?></span></button>
			</div>
		</div>
