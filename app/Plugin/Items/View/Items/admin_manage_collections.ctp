...
<ul class="breadcrumb top-mspace ver-space">
              <li><?php echo $this->Html->link( 'Items', array('controller'=>'items','action'=>'index'), array('escape' => false));?> <span class="divider">/</span></li>
...
<?php echo $this->Form->create('Collection', array('class' => 'form-horizontal space collectioneditform', 'action'=>'add_collection')); ?>
        <div class="clearfix sep-top">
           <div class="required pull-left span4 span4-sm no-mar ver-space dr mob-dl">
				<span class="hor-space show"><?php echo __l('Selected') . ' ' . Configure::read('item.alt_name_for_item_plural_caps'); ?></span>
			</div>
			<ul class="clearfix span ver-space unstyled no-mar">
                  <?php
                        ...
                        foreach($items as $item): ?>
                         <li>
                        <?php                        
                        echo $this->Html->link($this->Html->cText($item['Item']['title'], false), array('controller' => 'items', 'action' => 'view', $item['Item']['slug'],  'admin' => false), array('title'=>$this->Html->cText($item['Item']['title'], false),'escape' => false, 'class' => 'graydarkc'));
                         ?>
                        </li>
                   <?php $i++; endforeach; ?>
                </ul>
...
<?php
	echo $this->Form->input('item_list', array('type' => 'hidden', 'value' => $item_list));
?>
