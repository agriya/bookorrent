...
<ul class="breadcrumb top-mspace ver-space">
              ...
              <li class="active"><?php echo __l('Feedback To Host'); ?></li>
            </ul> 
<?php endif; ?>					
...
<?php if(empty($this->request->params['named']['simple_view'])) : ?>				
			<div class="clearfix dc">
					<?php echo $this->Form->create('ItemFeedback', array('type' => 'get', 'class' => 'form-search bot-mspace big-input span', 'action'=>'index')); ?>
...
<?php endif; ?>					
    ...
    <?php echo $this->Form->create('ItemFeedback' , array('class' => 'normal','action' => 'update')); ?>
...
foreach ($itemFeedbacks as $itemFeedback):
 $class = null;
	...
	if($itemFeedback['ItemFeedback']['is_satisfied']):
...
?>
	<tr<?php echo $class;?>>
        <?php if(empty($this->request->params['named']['simple_view'])) : ?>
		  <td class="dc"><?php echo $this->Form->input('ItemFeedback.'.$itemFeedback['ItemFeedback']['id'].'.id', array('type' => 'checkbox', 'id' => "admin_checkbox_".$itemFeedback['ItemFeedback']['id'], 'label' => "", 'class' => $status_class.' js-checkbox-list')); ?></td>
        <?php endif; ?>
		 <td class="dc"><span class="dropdown"> <span title="<?php echo __l('Actions');?>" data-toggle="dropdown" class="graydarkc left-space hor-smspace icon-cog text-18 cur dropdown-toggle"> <span class="hide"><?php echo __l('Actions');?></span> </span>
        			...
        			<li><?php echo $this->Html->link('<i class="icon-edit"></i>'.__l('Edit'), array('action' => 'edit', $itemFeedback['ItemFeedback']['id']), array('escape' => false,'class' => 'edit js-edit', 'title' => __l('Edit')));?></li>
        			<li><?php echo $this->Html->link('<i class="icon-remove"></i>'.__l('Delete'), array('action' => 'delete', $itemFeedback['ItemFeedback']['id']), array('escape' => false,'class' => 'delete js-delete', 'title' => __l('Delete')));?></li>
			   </ul>
   	  </span>
		</td>
		
		<td class="dc"><?php echo $this->Html->cDateTimeHighlight($itemFeedback['ItemFeedback']['created']);?></td>
		...
		<td class="dl">
		<?php 
		if(isset($itemFeedback['Item']['User'])){
			echo $this->Html->link($this->Html->cText($itemFeedback['Item']['User']['username']), array('controller'=> 'users', 'action'=>'view', $itemFeedback['Item']['User']['username'], 'admin' => false), array('class'=>'js-no-pjax', 'title' => $this->Html->cText($itemFeedback['Item']['User']['username'], false), 'escape' => false));
		}
		?>
		</td>
		<td class="dl"><?php echo $this->Html->link($this->Html->cText($itemFeedback['ItemUser']['User']['username']), array('controller'=> 'users', 'action'=>'view', $itemFeedback['ItemUser']['User']['username'], 'admin' => false), array('class'=>'js-no-pjax', 'title' => $this->Html->cText($itemFeedback['ItemUser']['User']['username'], false), 'escape' => false));?></td>
		<td class="dl"> <div class="htruncate span5 js-bootstrap-tooltip" title="<?php echo $this->Html->cText($itemFeedback['ItemFeedback']['feedback'], false); ?>"><?php echo $this->Html->cText($itemFeedback['ItemFeedback']['feedback']);?></td>
		<td class="dl">
		  <?php if(!empty($itemFeedback['Ip']['ip'])): ?>
                            ...
                            if(!empty($itemFeedback['Ip']['Country'])):
                                ?>
                                <span class="flags flag-<?php echo $this->Html->cText(strtolower($itemFeedback['Ip']['Country']['iso_alpha2']), false); ?>" title ="<?php echo $this->Html->cText($itemFeedback['Ip']['Country']['name'], false); ?>">
                            ...
                            ?>
                            <p class="htruncate js-bootstrap-tooltip span2 inline" title="<?php echo $this->Html->cText($itemFeedback['Ip']['City']['name'], false);?>"><?php echo $itemFeedback['Ip']['City']['name'];?>     </p>
                            <?php endif; ?></div>
		...
		<td class="dc"><?php echo $this->Html->cBool($itemFeedback['ItemFeedback']['is_satisfied']);?></td>
	</tr>
<?php
    endforeach;
?>
