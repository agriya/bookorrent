<?php /* SVN: $Id: $ */ ?>
<div class="categoryTypes index js-response">
	<ul class="breadcrumb top-mspace ver-space">
        ...
        <li class="active"><?php echo __l('Category Types'); ?></li>
	</ul> 
		...
		<div id="list" class="tab-pane active in no-mar">
			...
			<div class="clearfix dc">
				<div class="pull-right top-space mob-clr dc top-mspace">	 
					<?php echo $this->Html->link('<span class="ver-smspace"><i class="icon-plus-sign no-pad top-smspace"></i></span>', array('action' => 'add', $category_id), array('escape' => false,'class' => 'add btn btn-primary textb text-18 whitec','title'=>__l('Add'))); ?>
				</div>
			</div>
			<?php } ?>
			<?php 
				echo $this->element('paging_counter');
				echo $this->Form->create('CategoryType' , array('class' => 'normal','action' => 'update'));
					...
					<table class="table no-round table-striped">
						<thead>
							<tr class=" well no-mar no-pad">
								...
								<th class="dl sep-right"><?php echo $this->Paginator->sort('category_id', __l('Category'));?></th>
								<th class="dl sep-right"><?php echo $this->Paginator->sort('name', __l('Name'));?></th>
								<th class="dl sep-right"><?php echo $this->Paginator->sort('is_active', __l('Enabled?'));?></th>
							</tr>
						</thead>
						<tbody>
							<?php
							if (!empty($category_types)):
								...
								foreach ($category_types as $category_type):
									...
									if($category_type['CategoryType']['is_active']):
							...
							<tr class="<?php echo $class.' '.$active_class;?>">
								<td class="dc"><?php echo $this->Form->input('CategoryType.'.$category_type['CategoryType']['id'].'.id', array('type' => 'checkbox', 'id' => "admin_checkbox_".$category_type['CategoryType']['id'], 'label' => "", 'class' => $status_class.' js-checkbox-list')); ?></td>
								<td class="dc">
									<span class="dropdown"> 
										...
										<ul class="dropdown-menu arrow no-mar dl">
											<li><?php echo $this->Html->link('<i class="icon-edit"></i>'.__l('Edit'), array('action' => 'edit', $category_type['CategoryType']['id']), array('escape' => false,'class' => 'delete', 'title' => __l('Edit')));?></li>
											<li><?php echo $this->Html->link('<i class="icon-remove"></i>'.__l('Delete'), array('action' => 'delete', $category_type['CategoryType']['id']), array('escape' => false,'class' => 'delete js-delete', 'title' => __l('Delete')));?></li>
										</ul>
									</span>
								</td>
								<td class="dc"><?php echo $this->Html->cDateTime($category_type['CategoryType']['created']);?></td>
								<td class="dl"><?php echo $this->Html->cText($category_type['Category']['name']);?></td>
								<td class="dl"><?php echo $this->Html->cText($category_type['CategoryType']['name']);?></td>
								<td class="dl"><?php echo $this->Html->cBool($category_type['CategoryType']['is_active']);?></td>
							</tr>
							<?php
								endforeach;
							else:
							?>
							<tr>
								<td colspan="5"<div class="space dc grayc"><p class="ver-mspace top-space text-16"><?php echo __l('No Category Types available');?></p></div></td>
							</tr>
							<?php
								endif;
							?>
