    	...
    	<?php
			if($this->Auth->sessionValid() && ($this->Auth->user('role_id') == ConstUserTypes::Admin)):
				if($message['MessageContent']['is_system_flagged']):
					echo $this->Html->link(__l('Clear flag'), array('action' => 'update_status', $message['MessageContent']['id'], 'flag' => 'deactivate', 'admin' => true), array('class' => 'clear-flag js-admin-update-item', 'title' => __l('Clear flag')));
				else:
					echo $this->Html->link(__l('Flag'), array('action' => 'update_status', $message['MessageContent']['id'], 'flag' => 'active', 'admin' => true), array('class' => 'flag js-admin-update-item', 'title' => __l('Flag')));
				endif;
				if($message['User']['role_id'] != ConstUserTypes::Admin):
					if($message['User']['is_active']):
						echo $this->Html->link(__l('Deactivate sender'), array('controller' => 'users', 'action' => 'update_status', $message['User']['id'], 'status' => 'deactivate', 'admin' => true), array('class' => 'deactive-user js-admin-update-item', 'title' => __l('Deactivate user')));
					else:
						echo $this->Html->link(__l('Activate sender'), array('controller' => 'users', 'action' => 'update_status', $message['User']['id'], 'status' => 'activate', 'admin' => true), array('class' => 'active-user js-admin-update-item', 'title' => __l('Activate user')));
					endif;
				endif;
			endif;				
		?>
			...
			<?php if (!empty($is_starred)) : ?>
                ...
                ?>
					...
					<?php else:?>
						...
						<p><?php echo $this->Html->link(__l('Sent') . ' ' . Configure::read('item.alt_name_for_item_plural_small') , array('controller' => 'messages','action' => 'sentmail')); ?></p>
					<?php endif;?>					
                <?php endif;?>
				</div>
			</div>
			...
			<div class="message-block-right  clearfix">
              ...
              <div class="mail-body js-corner mail-content-curve-middle">
                ...
                <div class="mail-date-time sfont dr ">
                    <p class="<?php echo $this->Html->cInt($message['Message']['id'], false); ?> clearfix">
			   ...
			   <div class="js-show-mail-detail-div" style="display:none;">
				<?php
                    if ($message['Message']['is_sender'] == 0) : ?>
                    	<p  class="clearfix" ><span class="show-details-left dr message-title"><?php echo __l('from').': ';  ?></span> <?php echo $this->Html->cHtml($message['OtherUser']['username'], false); ?> < <?php echo $message['OtherUser']['email']; ?> ></p>
                    <?php
                    else : ?>
                    	<p  class="clearfix"><span class="show-details-left dr message-title"><?php echo __l('from').': ';  ?></span> <?php echo $this->Html->cHtml($message['User']['username'], false); ?> < <?php echo $message['User']['email']; ?> ></p>
        			<?php
                    endif; ?>
    				<p  class="clearfix"><span class="show-details-left dr message-title"><?php echo __l('to').': ';  ?></span><?php echo $this->Html->cText($show_detail_to, false); ?></p>
               	...
               	<p  class="clearfix">
					<span class="show-details-left dr message-title">
						<?php echo Configure::read('item.alt_name_for_item_singular_caps') . ': ';  ?>
					</span>
					<?php if(!empty($item['name'])):?>
						<?php echo $this->Html->link($this->Html->cText($item['name']), array('controller' => 'items', 'action' => 'view', $item['slug']), array('title' => $this->Html->cText($item['name'],false),'escape' => false));?>
					<?php endif;?>
				</p>
                <?php if(!empty($message['Message']['item_user_id'])):?>
              	<p  class="clearfix"><span class="show-details-left message-title dr"><?php echo __l('Booking').': ';  ?></span>
                   	<?php echo $this->Html->cText($message['Message']['item_user_id']); ?>
     			</p>
                <?php endif;?>
				...
				<?php
					if(!empty($message['ItemUser']['item_user_status_id']) && $message['ItemUser']['item_user_status_id'] == ConstItemUserStatus::WaitingforAcceptance && $message['Item']['user_id'] == $message['Message']['user_id']):
						if (!empty($message['ItemUser']['Attachment'])) :?>
							<?php echo '<p>'.__l('Attachment from buyer').':</p>';?>
							<span><?php echo $this->Html->link($message['ItemUser']['Attachment'][0]['filename'], array('controller' => 'item_users', 'action' => 'download', $message['ItemUser']['Attachment'][0]['id']), array('title' => __l('Click to download this file').': '.$message['ItemUser']['Attachment'][0]['filename'] )); ?></span>
						<?php endif;?>
						<?php if (!empty($message['ItemUser']['information_from_buyer'])) :?>
							<?php echo '<p>'.__l('Information from buyer').':</p>';?>
							<?php echo '<p>'.$message['ItemUser']['information_from_buyer'].'</p>';?>
						<?php endif;?>
						<?php
					endif;
				?>
