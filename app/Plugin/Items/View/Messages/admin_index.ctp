				...
				?>
					...
					<?php
						...
						<?php
							...
							echo "<span class='hor-space'>".$this->Form->autocomplete('Item.title', array('label'=>false,'div'=>false,'placeholder'=> Configure::read('item.alt_name_for_item_singular_caps'), 'acFieldKey' => 'Item.id', 'acFields' => array('Item.title'), 'acSearchFieldNames' => array('Item.title'), 'maxlength' => '255', 'class' => 'span5'))."</span>";
...
<div class="ver-space">
	...
	<th class="dl sep-right"><?php echo Configure::read('item.alt_name_for_item_singular_caps'); ?></th>
...
foreach($messages as $message):
   // if empty subject, showing with (no suject) as subject as like in gmail
    if (!$message['MessageContent']['subject']) :
		$message['MessageContent']['subject'] = __l('(no subject)');
    endif;
	if ($i++ % 2 == 0) :
		$row_class = 'row';
	else :
...
?>
    <tr>
		...
		<td class="w-one dc">
				<?php echo $this->Form->input('Message.'.$message['Message']['id'].".id", array('type' => 'checkbox', 'id' => 'admin_checkbox_'.$message['Message']['id'], 'label' => "", 'class' => $message_class.' js-checkbox-list'));?>
		</td>
        ...
        <td  class="dl <?php echo $row_three_class;?>">
          <div class="clearfix">
				 <div class="htruncate js-bootstrap-tooltip span5" title="<?php echo $this->Html->cText($message['MessageContent']['subject'], false);?>">
		...
		<td class="dl">
		<div class="htruncate js-bootstrap-tooltip span5" title="<?php echo $this->Html->cText($message['Item']['title'], false);?>">
			<?php
				if(!empty($message['Item']['title'])):
					echo $this->Html->link($this->Html->cText($message['Item']['title'], false), array('controller' => 'items', 'action' => 'view', $message['Item']['slug'], 'admin' => false), array('title' => $this->Html->cText($message['Item']['title'], false), 'class' => 'js-no-pjax', 'escape' => false));
				else:
					echo '-';
				endif;
			?>
...
if (!empty($messages)):
       ...
       <span class="graydarkc">
			...
			<?php echo $this->Html->link(__l('All'), '#', array('class' => 'graydarkc hor-smspace js-select js-no-pjax {"checked":"js-checkbox-list"}', 'title' => __l('All'))); ?>
			<?php echo $this->Html->link(__l('None'), '#', array('class' => 'graydarkc hor-smspace  js-select js-no-pjax {"unchecked":"js-checkbox-list"}', 'title' => __l('None'))); ?>
			<?php echo $this->Html->link(__l('Flagged'), '#', array('class' => 'graydarkc hor-smspace js-select js-no-pjax {"checked":"js-checkbox-flagged","unchecked":"js-checkbox-unflagged"} ', 'title' => __l('Flagged'))); ?>
			<?php echo $this->Html->link(__l('Unflagged'), '#', array('class' => ' graydarkc hor-smspace js-select js-no-pjax {"checked":"js-checkbox-unflagged","unchecked":"js-checkbox-flagged"}', 'title' => __l('Unflagged'))); ?>
			<?php echo $this->Html->link(__l('Suspended'), '#', array('class' => 'graydarkc hor-smspace js-select js-no-pjax	{"checked":"js-checkbox-suspended","unchecked":"js-checkbox-unsuspended"}', 'title' => __l('Suspended'))); ?>
