...
<section class="span bot-mspace user-dashboard sep mob-sep-none mob-no-pad message-block tab-no-mar span22">
    ...
    <div class="mail-main-curve">
			...
			<?php
                if (!empty($is_starred)) :
                    ...
                    echo $this->Html->link(__l('Back to') . ' ' . __l($back_link_msg), array('controller' => 'messages','action' => $folder_type), array('class' => 'backto'));
                endif;
            ?>
			</p>
			   ...
			   <div class="js-show-mail-detail-div show-mail">
				...
				<?php
                    if ($message['Message']['is_sender'] == 0) : ?>
						...
						<dd class="bot-space"><?php echo $this->Html->cText($message['OtherUser']['username'], false); ?></dd>	  
                    <?php
                    else : ?>
						<dt class="bot-space"><?php echo __l('From').': ';  ?></dt>
						<dd class="bot-space"><?php echo $this->Html->cText($message['User']['username'], false); ?> < <?php echo $message['User']['email']; ?> ></dd>
        			<?php
                    endif; ?>
						<dt class="bot-space"><?php echo __l('To').': ';  ?></dt>
						<dd class="bot-space"><?php echo $this->Html->cText($show_detail_to, false); ?></dd>
						...
						<?php if (!empty($message['Message']['item_id'])) :?>
						<dt class="bot-space"><?php echo Configure::read('item.alt_name_for_item_singular_caps') . ': '; ?></dt>
						<dd class="bot-space"><?php echo $this->Html->link($message['Item']['title'], array('controller' => 'items', 'action' => 'view', $message['Item']['slug']), array('title' => $message['Item']['title']));?></dd>
						<?php endif;?>
					...
					<?php if (!empty($message['Message']['item_user_id'])) :?>
						...
						<dd class="bot-space"><?php echo $this->Html->link($message['Message']['item_user_id'], array('controller' => 'messages', 'action' => 'activities', 'order_id' => $message['Message']['item_user_id']), array('title' => $message['Message']['item_user_id']));?></dd>
					<?php endif; ?>
                ...
                <div class="message-inner-content">
					<?php
						...
						if (!empty($message['Message']['item_user_status_id']) && $message['Message']['item_user_status_id'] != ConstItemUserStatus::BookerConversation && $message['Message']['item_user_status_id'] != ConstItemUserStatus::PrivateConversation && $message['Message']['item_user_status_id'] != ConstItemUserStatus::BookingRequestConversation):
    ...
    if (!empty($is_starred)) :
        ...
        echo $this->Html->link(__l('Back to') . ' ' . __l($back_link_msg), array(
            'controller' => 'messages',
            'action' => $folder_type
        ), array('class' => 'backto'));
    endif;
