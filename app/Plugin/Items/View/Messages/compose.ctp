...
?>
<div class="span24">
    <div class="clearfix mspace  form-compose">
    <label>
        <?php 	echo __l('From'); ?>
    </label>
 ...
 <div class="clearfix mspace  form-compose">
     <label>
	  ...
	  <?php echo !empty($this->request->data['Message']['to_username']) ? $this->Html->link($this->Html->cText($this->request->data['Message']['to_username']), array('controller'=> 'users', 'action' => 'view', $this->request->data['Message']['to_username']), array('title' => $this->Html->cText($this->request->data['Message']['to_username'],false),'escape' => false)) : 'Guest'; ?>
 </div>
    <?php if (!empty($this->request->data['Message']['item_name']) || !empty($this->request->data['Message']['request_name'])): ?>
    <div class="clearfix bot-mspace top-mspace">
		<label class=" message-title-left dr">
			<?php echo $this->request->data['Message']['from'];  ?>
		</label>
		<?php if($this->request->data['Message']['from']=='Item'): ?>
		<?php echo $this->Html->link($this->Html->cText($this->request->data['Message']['item_name']), array('controller' => 'items', 'action' => 'view',  $this->request->data['Message']['item_slug']), array('title' => $this->Html->cText($this->request->data['Message']['item_name'],false),'escape' => false));?>
 ...
 <?php	endif;  ?>
 <?php if(!empty($this->request->params['named']['item_user_id']) || !empty($this->request->data['Message']['item_user_id'])):?>
	<p class="clearfix">
		<span class="message-title"><?php 	echo __l('Booking#'); ?></span>
		 <?php	if(!empty($this->request->params['named']['item_user_id'])){
				echo $this->request->params['named']['item_user_id'];
			}elseif(!empty($this->request->data['Message']['item_user_id'])){
				echo $this->request->data['Message']['item_user_id'];	
			}
		 ?>
	</p>
	<?php endif;?>
			...
			<?php
				...
				echo $this->Form->input('type', array('type' => 'hidden'));
					if(!empty($this->request->data['Message']['to_username'])):
						echo $this->Form->input('to_username', array('type' => 'hidden', 'id' => 'message-to','label' => __l('To')));
					...
					if (!empty($this->request->data['Message']['item_slug'])):
				...
				<?php 
					...
					if (!empty($this->request->data['Message']['item_name'])):
