...
//		}
	...
	public function fill () {
		...
		if (is_object(@$args[0])) {
			...
			return $this->err(__l('First argument needs to be a model'));
		}
...
//            }
	...
	public function execute ($Model, $method, $path, $payload = array(), $options = array()) {
		...
		if (($e = curl_error($conn))) {
			return sprintf(__l('Error from elasticsearch server while contacting %s (%s)'), $uri, $e);
		}
		if (false === $response) {
			return sprintf(__l('Invalid response from elasticsearch server while contacting %s (%s)'), $uri, $json);
		}
		if (@$response['error']) {
			if ($response['error'] === 'ActionRequestValidationException[Validation Failed: 1: no requests added;]') {
				return $response;
			}
			return sprintf(__l('Error from elasticsearch server while contacting %s (%s)'), $uri, @$response['error']);
		}
	...
	public function search () {
		...
		if (empty($LeadingModel)) {
			return $this->err(__l('First argument needs to be a valid model'));
		}
