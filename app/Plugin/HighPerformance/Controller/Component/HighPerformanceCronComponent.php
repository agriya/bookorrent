						...
						if (Configure::read('thumbs.' . $attachment['Attachment']['class']) != '') {
							...
							foreach(Configure::read('thumbs.' . $attachment['Attachment']['class']) as $thumb_type) {
								if (Configure::read('s3.keep_copy_in_local')) {
									...
									$fullPath = 'http://' . Configure::read('s3.end_point') . '/' . Configure::read('s3.bucket_name') . '/' . $attachment['Attachment']['dir'] . '/' . $attachment['Attachment']['filename'];
								...
								$thumbUrl[$thumb_type] = 'http://' . Configure::read('s3.end_point') . '/' . Configure::read('s3.bucket_name') . '/' . $thumb_type . '/' . $attachment['Attachment']['dir'] . '/' . $attachment['Attachment']['filename'];
								if (Configure::read('s3.is_cname_enabled')) {
