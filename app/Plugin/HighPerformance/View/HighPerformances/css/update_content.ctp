...
if (isPluginEnabled('ItemFavorites')) {
    if (!empty($_GET['pids'])) {
        ...
        if ($this->Auth->user('id')) {
            foreach($followingItemIds as $followingItems) {
                $fid[] = $followingItems['ItemFavorite']['item_id'];
            }
        }
        for ($i = 0; $i < count($pids); $i++) {
            if (!$this->Auth->user('id')) {
                ...
                $display_none_arr[] = 'alpuf-sm-' . $pids[$i];
                if (in_array($pids[$i], (array)$ownItemIds)) {
                    $display_none_arr[] = 'alpf-' . $pids[$i];
                    $display_none_arr[] = 'alpuf-' . $pids[$i];
...
if (!empty($_GET['pids'])) {
    if ($this->Auth->user('id')) {
        if (in_array($pids[0], (array)$ownItemIds)) {
...
// Item book it end here //
// Item Flag start here //
if (!empty($_GET['pids'])) {
    for ($i = 0; $i < count($pids); $i++) {
        if (!$this->Auth->user('id')) {
...
if (!empty($_GET['pids'])) {
    ...
    for ($i = 0; $i < count($pids); $i++) {
        if (!$this->Auth->user('is_facebook_friends_fetched')) {
            ...
            $display_none_arr[] = 'alfbr-fb-nl-' . $pids[$i];
            $display_none_arr[] = 'alfbr-fb-na-' . $pids[$i];
        } elseif (empty($item['User']['is_facebook_friends_fetched'])) {
            $display_none_arr[] = 'blfbr-' . $pids[$i];
            $display_none_arr[] = 'alfbr-fb-e-' . $pids[$i];
            $display_block_arr[] = 'alfbr-fb-d-' . $pids[$i];
            $display_none_arr[] = 'alfbr-fb-nl-' . $pids[$i];
            $display_none_arr[] = 'alfbr-fb-na-' . $pids[$i];
        } elseif (!empty($network_level[$item['Item']['user_id']])) {
            $display_none_arr[] = 'blfbr-' . $pids[$i];
            $display_none_arr[] = 'alfbr-fb-e-' . $pids[$i];
            $display_none_arr[] = 'alfbr-fb-d-' . $pids[$i];
