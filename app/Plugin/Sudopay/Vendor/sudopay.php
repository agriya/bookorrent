...
class SudoPay_API
{
    ...
    private function _safe_json_decode($json)
    {
        ...
        if ($return === null) {
            $error['error']['code'] = 1;
            $error['error']['message'] = __l('Syntax error, malformed JSON');
            return $error;
        }
        return $return;
    }
    private function _safe_xml_decode($xml)
    {
        ...
        if ($return === false) {
            $error['error']['code'] = 1;
            $error['error']['message'] = __l('Syntax error, malformed XML');
            return $error;
        }
    ...
    private function _execute($url, $method = 'get', $post = array())
    {
        ...
        switch ($http_code) {
			...
			case 401:
				$return['error']['code'] = 1;
				$return['error']['message'] = __l('Unauthorized');
				break;
			case 400:
			case 500:
			case 504: /* Here we're not sure if anything got already triggered/saved in SudoPay */
				$return['error']['code'] = 1;
				$return['error']['message'] = __l('Problem in gateway. Recheck later.');
				break;
			default:
				$return['error']['code'] = 1;
				$return['error']['message'] = __l('Not Found');
        }
        curl_close($ch);
        return $return;
    }
