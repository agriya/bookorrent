...
class ExtensionsPluginsController extends AppController
{
    ...
    public function admin_delete($plugin = null)
    {
        ...
        if ($result === true) {
            $this->Session->setFlash(__l('Plugin deleted'). ' ' . $plugin , 'default', null, 'success');
        } elseif (!empty($result[0])) {
            $this->Session->setFlash($result[0], 'default', null, 'error');
        } else {
            $this->Session->setFlash(__l('Plugin could not be deleted. Please, try again.'), 'default', null, 'error');
        }
        $this->redirect(array(
            'action' => 'index'
        ));
