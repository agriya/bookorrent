...
<?php } else {?>
	<h2 class="ver-space top-mspace text-32 sep-bot"><?php echo __l("Edit Partition"); ?></h2>
<?php } ?>
<?php 
...
<div class="container">
  <div class="row">
	<div class="span8 seat-form">
		<div class="text-16 mspace"><?php echo __l("Area Dimension"); ?></div>
		<div class="input text clearfix">
			<div class="span3">
			  <?php echo $this->Form->input('no_of_rows', array('label' => false, 'class' => 'input-formc span3', "placeholder"=>__l("Rows"), 'id' => 'seatRows', 'div' => array('class' => 'input text'))); ?>
			</div>
			<div class="span3">
			  <?php echo $this->Form->input('no_of_columns', array('label' => false, 'class' => 'input-formc span3', "placeholder"=>__l("Columns"), 'id' => 'seatCols', 'div' => array('class' => 'input text'))); ?>
			</div>
		</div>
		<?php 
	...
	<div class="span8">
	  ...
	  <div class="ver-mspace">
		...
		<div class="submit pull-right">
			<?php 
				echo $this->Form->submit(__l('Save'), array('class' => 'hor-mspace btn btn-primary textb text-16'));
			?>
		</div>
	</div>
...
<div class="js-seat-generate-responses">
	<?php 
	if(!empty($this->request->data['Partition']['no_of_rows']) && !empty($this->request->data['Partition']['no_of_columns']) && !empty($this->request->data['Partition']['seating_direction']) && !empty($this->request->data['Partition']['seating_name_type']) && !empty($this->request->data['Partition']['id'])){
		echo $this->requestAction(array('controller' => 'seats', 'action' => 'generate', 'rows' => $this->request->data['Partition']['no_of_rows'], 'cols' => $this->request->data['Partition']['no_of_columns'], 'direction' => $this->request->data['Partition']['seating_direction'], 'naming' => $this->request->data['Partition']['seating_name_type'], 'partition_id' => $this->request->data['Partition']['id'] ), array('return'));
	}
	?>
</div>
